...
public partial class Ast_kerosakan : System.Web.UI.Page
{
    ...
    void view_details()
    {
        ...
        try
        {
            try
            {
                ...
                dd_cmp = DBCon.Ora_Execute_table("select * from ast_complaint where cmp_id='" + lbl_name.Text + "'");
                ...
                if (ddokdicno.Rows.Count != 0)
                {
                    ...
                    {
                        if(dd_cmp.Rows[0]["cmp_approval_sts_cd"].ToString().Trim() != "")
                        {
    ...
    protected void clk_cetak(object sender, EventArgs e)
    {
        try
        {
            if (lbl_name.Text != "")
            {
                ...
                dt = DBCon.Ora_Execute_table("select cmp_id,cmp_staff_no,FORMAT(cmp_complaint_dt,'dd/MM/yyyy', 'en-us') as cmp_complaint_dt,cmp_remark,FORMAT(cmp_action_dt,'dd/MM/yyyy', 'en-us') as cmp_action_dt,cmp_action_remark, case when cmp_sts_cd = '01'  then 'BARU' when cmp_sts_cd= '02' then 'SEDANG DISELENGGARA' when cmp_sts_cd= '03' then 'SELESAI' when cmp_sts_cd= '04' then 'RUJUK PENYELIA' end as ss1 from ast_complaint where cmp_id='"+ lbl_name.Text +"'");
                Rptviwer_lt.Reset();
                ds.Tables.Add(dt);
                ...
                ReportDataSource rds = new ReportDataSource("ast_tinkero", dt);
                Rptviwer_lt.LocalReport.DataSources.Add(rds);
                Rptviwer_lt.LocalReport.ReportPath = "Aset/AST_Tin_kero.rdlc";
                //DataTable applcn = new DataTable();
                //applcn = dbcon.Ora_Execute_table("");
                string sv1 = string.Empty, sv2 = string.Empty;
                string dd1 = string.Empty, dd2 = string.Empty, dd3 = string.Empty, dd4 = string.Empty, dd5 = string.Empty;
                ...
                ReportParameter[] rptParams = new ReportParameter[]{
                     new ReportParameter("s1", lbl_name.Text),
...
//            BindGrid();
    ...
    protected void clk_smpn(object sender, EventArgs e)
    {
        ...
        if (ddicno1.Rows.Count != 0)
        {
            ...
            DateTime td1 = DateTime.ParseExact(TextBox9.Text, "dd/MM/yyyy", CultureInfo.InvariantCulture);
            string cmp_dt = td1.ToString("yyyy-MM-dd HH:mm:ss");

            // Update Query

            string Inssql1 = "update ast_complaint set cmp_sts_cd='" + DropDownList5.SelectedValue + "',cmp_maintain_ind='" + cb + "',cmp_action_remark='" + Textarea1.Value.Replace("'", "''") + "',cmp_cost_amt='" + txt_bx1.Text + "',cmp_action_id='" + Session["New"].ToString() + "',cmp_action_dt='" + cmp_dt + "',cmp_upd_id='" + Session["New"].ToString() + "',cmp_upd_dt='" + DateTime.Now + "' where cmp_id='" + TextBox1.Text + "'";
            Status = DBCon.Ora_Execute_CommamdText(Inssql1);
            if (Status == "SUCCESS")
            {
                Session["validate_success"] = "SUCCESS";
                Session["alrt_msg"] = "Rekod Berjaya Disimpan.";
                Response.Redirect("../Aset/Ast_kerosakan_view.aspx");
                //BindGrid();
            }
        }
        else
