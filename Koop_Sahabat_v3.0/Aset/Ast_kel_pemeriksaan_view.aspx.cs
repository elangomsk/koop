...
public partial class Ast_kel_pemeriksaan_view : System.Web.UI.Page
{
    ...
    {
        con.Open();
        qry1 = "select s1.sas_asset_id,s2.stf_name,s1.sas_qty,s3.ast_kategori_desc,Format(s1.sas_allocate_dt,'dd/MM/yyyy') sas_allocate_dt,case when Format(ISNULL(s1.sas_inspect_dt,''),'dd/MM/yyyy')='01/01/1900' then '' else Format(ISNULL(s1.sas_inspect_dt,''),'dd/MM/yyyy') end as inspect_dt,case when Format(ISNULL(s1.sas_approve_dt,''),'dd/MM/yyyy')='01/01/1900' then '' else Format(ISNULL(s1.sas_approve_dt,''),'dd/MM/yyyy') end as appr_dt,ISNULL(s4.ast_pemerikksaan_desc,'') cond_desc,ISNULL(s5.ast_justifikasi_desc,'') just_txt from ast_staff_asset s1 left join hr_staff_profile s2 on s2.stf_staff_no=s1.sas_staff_no left join Ref_ast_kategori s3 on s3.ast_kategori_code=s1.sas_asset_cat_cd  left join Ref_ast_sts_pemerikksaan s4 on s4.ast_pemerikksaan_code=s1.sas_cond_sts_cd left join Ref_ast_justifikasi s5 on s5.ast_justifikasi_code=s1.sas_justify_cd where ISNULL(s1.sas_inspect_dt,'') !=''";
