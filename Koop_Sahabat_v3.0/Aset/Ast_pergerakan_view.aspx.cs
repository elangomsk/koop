...
public partial class Ast_pergerakan_view : System.Web.UI.Page
{
    ...
    protected void BindData_jenis()
    {
        ...
        con.Open();
        SqlCommand cmd = new SqlCommand("select mov_asset_id,s1.stf_name,FORMAT(mov_verify_dt,'dd/MM/yyyy', 'en-us') as ver_dt,mov_verify_remark,case when mov_type_cd = '01' then 'Peminjaman' when mov_type_cd = '02' then 'Pemulangan' when mov_type_cd = '03' then 'Pindah Milik' else '' end as type_name from ast_borrow_txn  left join hr_staff_profile s1 on s1.stf_staff_no=mov_verify_id", con);
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ddokdicno = DBCon.Ora_Execute_table("select * from ast_borrow_txn where mov_asset_id='" + lblTitle.Text + "' ");
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../Aset/Ast_pergerakan.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
