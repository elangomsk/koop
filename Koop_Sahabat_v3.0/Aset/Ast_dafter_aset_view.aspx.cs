...
public partial class Ast_dafter_aset_view : System.Web.UI.Page
{
    ...
    protected void BindData_jenis()
    {
        ...
        con.Open();
        SqlCommand cmd = new SqlCommand("select *,case when ISNULL(a.appr_qty,'') != ISNULL(b.rec_qty,'') then 'Pending' when ISNULL(a.appr_qty,'') = ISNULL(b.rec_qty,'') then 'Delivered' else 'Waiting' end as sts from (select pur_po_no,pur_apply_staff_name,count(*) as cnt,sum(pur_asset_tot_amt) as tot_amt,sum(pur_asset_qty) apply_qty,sum(ISNULL(pur_verify_qty,'0')) as appr_qty from ast_purchase where ISNULL(pur_del_ind,'') != 'D' and pur_approve_sts_cd='01' group by pur_po_no,pur_apply_staff_name,pur_verify_sts_cd,pur_approve_sts_cd) as a left join (select acp_po_no,sum(acp_receive_qty) as rec_qty,acp_complete_ind,acp_verify_sts_cd from ast_acceptance group by acp_po_no,acp_complete_ind,acp_verify_sts_cd) as b on b.acp_po_no=a.pur_po_no where b.acp_complete_ind='01' and b.acp_verify_sts_cd='S'", con);
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../Aset/Ast_dafter_aset.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
