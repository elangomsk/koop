...
public partial class Ast_pak_aset_view : System.Web.UI.Page
{
    ...
    {
        con.Open();
        qry1 = "select lst_cmplnt_id,lst_asset_id,FORMAT(lst_incident_dt,'dd/MM/yyyy', 'en-us') inc_dt1,FORMAT(lst_cmplnt_dt,'dd/MM/yyyy', 'en-us') cmp_dt1,case when lst_verify_ind='N' then 'BARU' when lst_verify_ind='Y' then 'DISEMAK' else '' end as ver_sts from ast_lost where lst_staff_no='" + userid +"'";
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ddokdicno = DBCon.Ora_Execute_table("select * from ast_lost where lst_cmplnt_id='" + lblTitle.Text + "' ");
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../Aset/Ast_pak_aset.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
