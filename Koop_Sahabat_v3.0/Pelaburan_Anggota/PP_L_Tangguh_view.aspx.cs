...
public partial class PP_L_Tangguh_view : System.Web.UI.Page
{
    ...
    {
        con.Open();
        qry1 = "select *,case when hol_appl_sts_cd ='01' then 'PERMOHONAN' else 'BATAL PERMOHONAN' end as app_sts,case when hol_approve_sts_cd ='A' then 'LULUS' when hol_approve_sts_cd ='NA' then 'TOLAK' else '' end as approve_sts from jpa_payment_holiday";
    ...
    //{
    //    Response.Redirect("../Pelaburan_Anggota/PP_M_Tangguh.aspx");
    //}
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ddokdicno = DBCon.Ora_Execute_table("select * from jpa_payment_holiday where hol_applcn_no='" + lblTitle.Text + "' ");
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../Pelaburan_Anggota/PP_L_Tangguh.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
