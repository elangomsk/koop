...
public partial class PP_LP_Tempoh : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            if (Session["New"] != null)
            {
                ...
                Textarea2.Attributes.Add("Readonly", "Readonly");
                TextBox6.Attributes.Add("Readonly", "Readonly");
                TextBox51.Attributes.Add("Readonly", "Readonly");
        ...
        try
        {
            ...
            if (Applcn_no.Text != "")
            {
                ...
                if (ddicno.Rows.Count != 0)
                {
                    ...
                    DropDownList2.SelectedValue = select_app.Rows[0]["ext_approve_sts_cd"].ToString().Trim();
                    catatan.Value = select_app.Rows[0]["ext_approve_remark"].ToString();
                }
                else
                {
        ...
        try
        {
            if (Applcn_no.Text != "")
            {
                if (DropDownList2.SelectedValue != "" && catatan.Value != "")
                {
                    ...
                    string new_dur = TextBox6.Text + TextBox10.Text;

                    DataTable ddokdicno = new DataTable();
                    ddokdicno = Dblog.Ora_Execute_table("select * from jpa_extension where ext_applcn_no='" + cc_no + "'");
                    if (ddokdicno.Rows.Count > 0)
                    {
                        Dblog.Execute_CommamdText("Update jpa_extension SET ext_approve_sts_cd='" + DropDownList2.SelectedValue + "',ext_approve_remark='" + catatan.Value + "',ext_approve_id='" + Session["New"].ToString() + "',ext_approve_dt='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',ext_upd_id='" + Session["New"].ToString() + "',ext_upd_dt='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "',set_flag='0' WHERE ext_applcn_no='" + cc_no + "'");
                    }

                    Session["validate_success"] = "SUCCESS";
                    Session["alrt_msg"] = "Rekod Berjaya Dikemaskini.";
                    Response.Redirect("../Pelaburan_Anggota/PP_LP_Tempoh_view.aspx");
                }
                else
                {
