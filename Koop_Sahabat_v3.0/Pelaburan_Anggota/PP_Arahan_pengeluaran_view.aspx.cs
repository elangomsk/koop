...
public partial class PP_Arahan_pengeluaran_view : System.Web.UI.Page
{
    ...
    {
        con.Open();
        qry1 = "select distinct app_applcn_no,JA.app_new_icno,JA.app_name,RW.Wilayah_Name,ISNULL(RB.branch_desc, '') AS branch_desc, JJA.jkk_approve_amt, JJA.jkk_approve_dur from jpa_application as JA  Left Join jpa_jkkpa_approval as JJA ON JJA.jkk_applcn_no=JA.app_applcn_no Left Join Ref_Wilayah as RW ON RW.Wilayah_Code=JA.app_region_cd Left join ref_branch AS RB ON RB.branch_cd=JA.app_branch_cd inner join jpa_disburse db on db.pha_applcn_no=app_applcn_no ";
    ...
    {
        Response.Redirect("../Pelaburan_Anggota/PP_Arahan_pengeluaran.aspx");
    }
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ddokdicno = DBCon.Ora_Execute_table("select * from jpa_disburse where pha_applcn_no='" + lblTitle.Text + "' ");
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../Pelaburan_Anggota/PP_Arahan_pengeluaran.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
