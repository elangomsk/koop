...
public partial class PP_ktp_view : System.Web.UI.Page
{
    ...
    {
        con.Open();
        qry1 = "select pre_applcn_no,s2.app_name,app_new_icno, FORMAT(pre_lo_date,'dd/MM/yyyy', 'en-us') as dt1, FORMAT(pre_sign_loan_dt,'dd/MM/yyyy', 'en-us') as dt2, FORMAT(pre_sign_guarantor_dt1,'dd/MM/yyyy', 'en-us') as dt3, FORMAT(pre_agreement_dt,'dd/MM/yyyy', 'en-us') as dt4  from jpa_pre_disburse_checklist left join jpa_application s2 on s2.app_applcn_no=pre_applcn_no";
    ...
    {
        Response.Redirect("../Pelaburan_Anggota/PP_ktp.aspx");
    }
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ddokdicno = DBCon.Ora_Execute_table("select * from jpa_pre_disburse_checklist where pre_applcn_no='" + lblTitle.Text + "' ");
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../Pelaburan_Anggota/PP_ktp.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
