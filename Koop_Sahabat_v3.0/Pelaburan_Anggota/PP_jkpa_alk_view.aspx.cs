...
public partial class PP_jkpa_alk_view : System.Web.UI.Page
{
    ...
    {
        con.Open();
        qry1 = "select s2.app_applcn_no,s2.app_new_icno,s2.app_name,RT.tujuan_desc,s2.app_apply_amt,s2.app_apply_dur,RW.Wilayah_Name,ISNULL(RB.branch_desc, '') AS branch_desc from jpa_jkpa_approval left join jpa_application s2 on s2.app_applcn_no=jkp_applcn_no Left Join Ref_Wilayah as RW ON RW.Wilayah_Code=s2.app_region_cd left join ref_tujuan as RT ON RT.tujuan_cd=s2.app_loan_purpose_cd Left join ref_branch AS RB ON RB.branch_cd=s2.app_branch_cd";
    ...
    {
        Response.Redirect("../Pelaburan_Anggota/PP_jkpa_alk.aspx");
    }
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../Pelaburan_Anggota/PP_jkpa_alk.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
