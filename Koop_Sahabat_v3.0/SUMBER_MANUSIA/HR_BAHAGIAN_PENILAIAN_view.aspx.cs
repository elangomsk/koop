...
public partial class HR_BAHAGIAN_PENILAIAN_view : System.Web.UI.Page
{
    ...
    void app_language()
    {
        if (Session["New"] != null)
        {
            ...
            gt_lng = DBCon.Ora_Execute_table("select " + Session["site_languaage"].ToString() + " from Ref_language where ID IN ('528','448','133','39')");
    ...
    protected void BindData_Grid()
    {
        ...
        SqlCommand cmd = new SqlCommand("select cse_section_cd,cse_section_desc,cse_info,cse_marks From hr_cmn_appr_section", con);
        ...
        if (rcount != "0")
        {
            foreach (GridViewRow row in GridView1.Rows)
            {
                ...
                if (rbn.Checked)
                {
                    ...
                    string varName1 = ((System.Web.UI.WebControls.Label)row.FindControl("Label3")).Text.ToString(); //this store the  value in varName1
                    
                    string Delsql = "Delete from hr_cmn_appr_section where cse_section_cd='" + varName1 + "'";
                    Status = DBCon.Ora_Execute_CommamdText(Delsql);
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ddokdicno = DBCon.Ora_Execute_table("select * From hr_cmn_appr_section where cse_section_cd='" + lblTitle.Text + "' ");
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../SUMBER_MANUSIA/HR_BAHAGIAN_PENILAIAN.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
