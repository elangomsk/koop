...
public partial class HR_KM_pen_staff : System.Web.UI.Page
{
    ...
    protected void click_insert(object sender, EventArgs e)
    {
        try
        {
            if (Kaki_no.Text != "")
            {
                ...
                {
                        string markah1 = ((System.Web.UI.WebControls.TextBox)dataItem.FindControl("lbl_ula")).Text.Trim();
                    ...
                    if (markah1 != "0")
                    {
                        ...
                        {
                            string Inssql1 = "Update hr_staff_appraisal set sap_staff_remark='" + markah1 + "' where sap_staff_no='" + Kaki_no.Text + "' and sap_section_cd ='" + bah_cd1 + "' and sap_post_cat_cd='" + sub_cd1 + "' and cse_sec_type='" + sec_type1 + "' and sap_year='" + txt_tahun.Text + "'";
    ...
    void grid_kpr()
    {
            ...
            SqlCommand cmd2 = new SqlCommand("select * from (SELECT sap_section_cd,sap_post_cat_cd,sap_subject_cd,sap_staff_remark,sap_weightage,b.cse_section_desc,A.cse_sec_type,case when ISNULL(sap_ppp_score,'0')='0' then '0' else sap_ppp_score end as sap_ppp_score FROM hr_staff_appraisal A INNER JOIN hr_cmn_appr_section B ON A. sap_section_cd=B.cse_section_cd WHERE sap_staff_no='" + Kaki_no.Text + "' AND sap_year='" + txt_tahun.Text + "' AND cap_status='A' and A.cse_sec_type='02' ) as a "
                                            + " outer apply(SELECT sap_section_cd, sum(sap_weightage) cnt FROM hr_staff_appraisal A INNER JOIN hr_cmn_appr_section B ON A.sap_section_cd = B.cse_section_cd WHERE sap_staff_no = '" + Kaki_no.Text + "' AND sap_year = '" + txt_tahun.Text + "' AND cap_status = 'A' and A.cse_sec_type='02' group by sap_section_cd) as b where b.sap_section_cd = a.sap_section_cd ORDER BY a.sap_section_cd, a.sap_post_cat_cd", con);
