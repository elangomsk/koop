...
public partial class HR_ORGNISASI_view : System.Web.UI.Page
{
    ...
    void app_language()
    {
        ...
        gt_lng = DBCon.Ora_Execute_table("select " + Session["site_languaage"].ToString() + " from Ref_language where ID IN ('466','448','133','39')");
    ...
    protected void BindData_Grid()
    {
        ...
        SqlCommand cmd = new SqlCommand("select org_gen_id,org_id,org_name,org_contact_name1,org_contact_name2,org_state_cd from hr_organization ORDER BY org_crt_dt ASC", con);
    ...
    protected void btn_hups_Click(object sender, EventArgs e)
    {
        ...
        if (rcount != "0")
        {
            ...
            {
                var rbn = row.FindControl("chkRow") as System.Web.UI.WebControls.CheckBox;
                if (rbn.Checked)
                {
                    int RowIndex = row.RowIndex;
                    string varName1 = ((System.Web.UI.WebControls.Label)row.FindControl("Label7")).Text.ToString();
                    string varName2 = ((System.Web.UI.WebControls.Label)row.FindControl("Label3")).Text.ToString();//this store the  value in varName1
                    SqlCommand ins_peng = new SqlCommand("Delete from hr_organization where org_gen_id='" + varName1 + "'", conn);
                    conn.Open();
                    int i = ins_peng.ExecuteNonQuery();
                    conn.Close();
                    service.audit_trail("P0100", "Hapus", "Nama Organisasi", varName2);
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            System.Web.UI.WebControls.Label lblTitle = (System.Web.UI.WebControls.Label)gvRow.FindControl("Label7");
            string lblid1 = lblTitle.Text;
            DataTable ddokdicno = new DataTable();
            ddokdicno = DBCon.Ora_Execute_table("select * From hr_organization where org_gen_id='" + lblTitle.Text + "' ");
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../SUMBER_MANUSIA/HR_ORGNISASI.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
