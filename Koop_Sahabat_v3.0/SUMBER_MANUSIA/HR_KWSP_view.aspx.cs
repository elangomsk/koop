...
public partial class HR_KWSP_view : System.Web.UI.Page
{
    ...
    protected void BindData_Grid()
    {
        ...
        SqlCommand cmd = new SqlCommand("select cse_section_cd,cse_section_desc,cse_info From hr_cmn_appr_section", con);
    ...
    {
        Response.Redirect("../SUMBER_MANUSIA/HR_KWSP.aspx");
    }
        ...
        if (rcount != "0")
        {
            foreach (GridViewRow row in GridView1.Rows)
            {
                ...
                if (rbn.Checked)
                {
                    ...
                    SqlCommand ins_peng = new SqlCommand("Delete from hr_organization_jaba where id='" + varName1 + "'", conn);
                    conn.Open();
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ddokdicno = DBCon.Ora_Execute_table("select * From hr_organization_jaba where Id='" + lblTitle.Text + "' ");
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../SUMBER_MANUSIA/HR_KWSP.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
