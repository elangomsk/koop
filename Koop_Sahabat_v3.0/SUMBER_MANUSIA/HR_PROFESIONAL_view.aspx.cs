...
public partial class HR_PROFESIONAL_view : System.Web.UI.Page
{
    ...
    void app_language()
    {
        ...
        gt_lng = DBCon.Ora_Execute_table("select " + Session["site_languaage"].ToString() + " from Ref_language where ID IN ('486','448')");
    ...
    void assgn_roles()
    {
        ...
        if (ddokdicno.Rows.Count != 0)
        {
            ...
            ddokdicno_1 = DBCon.Ora_Execute_table("select m1.*,s1.ctrl_type from KK_Role_skrins m1 left join KK_PID_Kumpulan s1 on s1.KK_kumpulan_id=Role_id  where psub_skrin_id='P0088' and Role_id IN ('" + ddokdicno.Rows[0]["KK_roles"].ToString().Replace(",", "','") + "') order by Role_id");
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../SUMBER_MANUSIA/HR_PROFESIONAL.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
