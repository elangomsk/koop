...
public partial class HR_kem_mak_Latihan_view : System.Web.UI.Page
{
    ...
    void app_language()
    {
        if (Session["New"] != null)
        {
            ...
            gt_lng = DBCon.Ora_Execute_table("select " + Session["site_languaage"].ToString() + " from Ref_language where ID IN ('555','448','39')");
    ...
    protected void BindData_Grid()
    {
        ...
        SqlCommand cmd = new SqlCommand("select atd_staff_no,atd_date  from hr_attendance  " + sqry + " order by atd_staff_no asc", con);
    ...
    {
        Response.Redirect("../SUMBER_MANUSIA/HR_kem_mak_Latihan.aspx");
    }
        ...
        if (rcount != "0")
        {
            foreach (GridViewRow row in GridView1.Rows)
            {
                ...
                if (rbn.Checked)
                {
                    ...
                    SqlCommand ins_peng = new SqlCommand("Delete from hr_attendance where id='" + varName1 + "'", conn);
                    conn.Open();
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ddokdicno = DBCon.Ora_Execute_table("select * From hr_attendance where Id='" + lblTitle.Text + "' ");
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../SUMBER_MANUSIA/HR_LR_kehadiran.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
