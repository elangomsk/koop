...
public partial class HR_LM_latihan_view : System.Web.UI.Page
{
    ...
    protected void BindData_Grid()
    {
        ...
        SqlCommand cmd = new SqlCommand("select sp.stf_staff_no,upper(sp.stf_name) as stf_name,sum(ht.trn_dur) as tot_dur,hu.hr_jaba_desc from hr_training as ht left join hr_post_his as ph on ht.trn_staff_no=ph.pos_staff_no  left join hr_staff_profile as sp on sp.stf_staff_no=ht.trn_staff_no left join Ref_hr_jabatan as hu on hu.hr_jaba_Code=ph.pos_dept_cd   group by sp.stf_staff_no,sp.stf_name,hu.hr_jaba_desc",con);
    ...
    {
        Response.Redirect("../SUMBER_MANUSIA/HR_LM_latihan.aspx");
    }
        ...
        if (rcount != "0")
        {
            foreach (GridViewRow row in GridView1.Rows)
            {
                ...
                if (rbn.Checked)
                {
                    ...
                    SqlCommand ins_peng = new SqlCommand("Delete from hr_staff_profile where id='" + varName1 + "'", conn);
                    conn.Open();
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ddokdicno = DBCon.Ora_Execute_table("select * From hr_staff_profile where Id='" + lblTitle.Text + "' ");
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../SUMBER_MANUSIA/HR_LM_latihan.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
