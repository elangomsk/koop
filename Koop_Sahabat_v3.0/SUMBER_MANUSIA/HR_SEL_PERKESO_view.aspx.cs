...
public partial class HR_SEL_PERKESO_view : System.Web.UI.Page
{
    ...
    protected void BindData_Grid()
    {
        ...
        SqlCommand cmd = new SqlCommand("SELECT id,per_min_income_amt,per_max_income_amt,per_employer_amt1,per_employee_amt,per_employer_amt2 FROM hr_comm_perkeso ", con);
    ...
    {
        Response.Redirect("../SUMBER_MANUSIA/HR_SEL_PERKESO.aspx");
    }
        ...
        if (rcount != "0")
        {
            foreach (GridViewRow row in GridView1.Rows)
            {
                ...
                if (rbn.Checked)
                {
                    ...
                    SqlCommand ins_peng = new SqlCommand("Delete from hr_comm_perkeso where id='" + varName1 + "'", conn);
                    conn.Open();
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ddokdicno = DBCon.Ora_Execute_table("select * From hr_comm_perkeso where Id='" + lblTitle.Text + "' ");
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../SUMBER_MANUSIA/HR_SEL_PERKESO.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
