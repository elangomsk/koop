...
public partial class HR_SEMAKAN_view : System.Web.UI.Page
{
    ...
    protected void BindData_Grid()
    {
        ...
        ddokdicno = DBCon.Ora_Execute_table("select STUFF ((select ',''' + pos_staff_no + '''' from hr_post_his where pos_end_dt='9999-12-31' and '" + Session["New"].ToString() + "' IN (pos_spv_name2)  FOR XML PATH ('')  ),1,1,'')  as scode");
        ...
        SqlCommand cmd2 = new SqlCommand("select ph.Id,sp.stf_staff_no,sp.stf_name,FORMAT(lap_application_dt,'dd/MM/yyyy') lap_application_dt,lap_leave_type_cd,hjc.hr_jenis_desc,lap_leave_day,FORMAT(lap_leave_start_dt,'dd/MM/yyyy') as lap_leave_start_dt,FORMAT(lap_leave_end_dt, 'dd/MM/yyyy') as lap_leave_end_dt, lap_ref_no,case when lap_approve_sts_cd = '00' then 'MOHON' when lap_approve_sts_cd = '01' then 'SAH' when lap_approve_sts_cd = '02' then 'TIDAk SAH' when lap_approve_sts_cd = '03' then 'TIDAK LULUS' when lap_approve_sts_cd = '04' then 'BATAL' else '' end as app_stscd,case when lap_endorse_sts_cd = '01' then 'LULUS' when lap_endorse_sts_cd = '02' then 'TIDAK LULUS' else 'WAITING' end as psts,lap_late_apply,sp1.stf_name as approve_name from hr_leave_application ph left join hr_staff_profile sp on sp.stf_staff_no = ph.lap_staff_no Left join Ref_hr_jenis_cuti hjc on hjc.hr_jenis_Code = ph.lap_leave_type_cd left join hr_staff_profile as sp1 on sp1.stf_staff_no=ph.lap_approve_id where sp.stf_service_end_dt>='" + DateTime.Now.ToString("yyyy-MM-dd") + "' and  sp.stf_service_end_dt <= '9999-12-31' and ph.lap_cancel_ind='N' and ISNULL(lap_approve_sts_cd,'') ='01' and ISNULL(lap_endorse_sts_cd,'')='' and lap_staff_no IN(" + ssno + ") order by lap_ref_no", con);
