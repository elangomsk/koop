...
public partial class HR_SEMAK_HAJI_view : System.Web.UI.Page
{
    ...
    protected void BindData_Grid()
    {
        ...
        SqlCommand cmd = new SqlCommand("select inc_staff_no,a1.stf_name,a1.stf_icno,a2.Bank_Name,a1.stf_bank_acc_no,sum(inc_nett_amt) as namt from hr_income left join hr_staff_profile as a1 on a1.stf_staff_no=inc_staff_no left join Ref_Nama_Bank a2 on a2.Bank_Code=a1.stf_bank_cd  group by inc_staff_no,a1.stf_name,a1.stf_icno,a2.Bank_Name,a1.stf_bank_acc_no", con);
        ...
        if (rcount != "0")
        {
            foreach (GridViewRow row in GridView1.Rows)
            {
                ...
                if (rbn.Checked)
                {
                    ...
                    SqlCommand ins_peng = new SqlCommand("Delete from hr_income where id='" + varName1 + "'", conn);
                    conn.Open();
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ddokdicno = DBCon.Ora_Execute_table("select * From hr_income where Id='" + lblTitle.Text + "' ");
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../SUMBER_MANUSIA/HR_SEMAK_HAJI.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
