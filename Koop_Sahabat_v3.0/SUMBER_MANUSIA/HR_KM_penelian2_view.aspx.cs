...
public partial class HR_KM_penelian2_view : System.Web.UI.Page
{
    ...
    protected void BindData_Grid()
    {
        ...
        SqlCommand cmd = new SqlCommand("select stf_staff_no,stf_icno,stf_name,s3.hr_jaw_desc from hr_staff_profile s1 inner join hr_post_his s2 on s2.pos_staff_no=s1.stf_staff_no left join Ref_hr_Jawatan s3 on s3.hr_jaw_Code=s1.stf_curr_post_cd where s2.pos_end_dt='9999-12-31' and s2.pos_spv_name2 = '"+ userid +"'", con);
    ...
    protected void lnkView_Click1(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect("../SUMBER_MANUSIA/HR_KM_penelian2_pen.aspx?edit=" + name + "&&type=" + type + "");
            }
        }
        catch (Exception ex)
        {
    ...
    protected void lnkView_Click2(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect("../SUMBER_MANUSIA/HR_KM_penelian_lap.aspx?edit=" + name + "&&type=" + type + "&&pg_type=2");
            }
        }
        catch (Exception ex)
        {
