...
public partial class HR_MUK_view : System.Web.UI.Page
{
    ...
    protected void BindData_Grid()
    {
        ...
        SqlCommand cmd = new SqlCommand("select jph.pos_staff_no,rj.hr_jaw_desc,ru.hr_unit_desc,ISNULL(rjb.hr_jaba_desc,'') as hr_jaba_desc,FORMAT(jph.pos_start_dt,'dd/MM/yyyy', 'en-us') as pos_start_dt,jph.pos_end_dt,jph.pos_grade_cd,FORMAT(jph.pos_crt_dt,'yyyy-MM-dd HH:mm:ss', 'en-us') as pos_crt_dt,jph.pos_subjek from hr_post_his as jph left join hr_staff_profile as hsp on hsp.stf_staff_no = jph.pos_staff_no left join Ref_hr_Jawatan as rj on rj.hr_jaw_Code=jph.pos_post_cd left join Ref_hr_unit as ru on ru.hr_unit_Code=jph.pos_unit_cd left join Ref_hr_jabatan as rjb on rjb.hr_jaba_Code=jph.pos_dept_cd where jph.pos_staff_no='' ORDER BY jph.pos_start_dt desc", con);
    ...
    {
        Response.Redirect("../SUMBER_MANUSIA/HR_MUK.aspx");
    }
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../SUMBER_MANUSIA/HR_MUK.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
