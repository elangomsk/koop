...
public partial class HR_KM_penelian_lap_view : System.Web.UI.Page
{
    ...
    void bind_year()
    {
        ...
        try
        {
            ...
            string com = "SELECT sap_year FROM hr_staff_appraisal GROUP BY sap_year";
            SqlDataAdapter adpt = new SqlDataAdapter(com, con);
            DataTable dt = new DataTable();
            adpt.Fill(dt);

            txt_tahun.DataSource = dt;
            txt_tahun.DataBind();
            txt_tahun.DataTextField = "sap_year";
            txt_tahun.DataValueField = "sap_year";
            txt_tahun.DataBind();
            txt_tahun.Items.Insert(0, new ListItem("--- PILIH ---", ""));
        }
        catch (Exception ex)
        {
            throw ex;
    ...
    protected void BindData_Grid()
    {
        ...
        SqlCommand cmd = new SqlCommand("select stf_staff_no,stf_icno,stf_name,s3.hr_jaw_desc from hr_staff_profile s1 inner join hr_post_his s2 on s2.pos_staff_no=s1.stf_staff_no inner join  hr_staff_appraisal ss1 on ss1.sap_staff_no=s1.stf_staff_no left join Ref_hr_Jawatan s3 on s3.hr_jaw_Code=s1.stf_curr_post_cd where s2.pos_end_dt='9999-12-31' and ss1.sap_year='"+ txt_tahun.SelectedValue +"' group by stf_staff_no,stf_icno,stf_name,s3.hr_jaw_desc", con);
    ...
    protected void chg_tahun(object sender, EventArgs e)
    {
        BindData_Grid();
    ...
    protected void lnkView_Click2(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                string type = HttpUtility.UrlEncode(service.Encrypt(txt_tahun.SelectedValue));
                Response.Redirect("../SUMBER_MANUSIA/HR_KM_penelian_lap.aspx?edit=" + name + "&&type=" + type + "&&pg_type=3");
            }
        }
        catch (Exception ex)
        {
