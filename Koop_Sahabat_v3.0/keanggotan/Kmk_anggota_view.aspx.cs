...
public partial class Kmk_anggota_view : System.Web.UI.Page
{
    ...
    {
        con.Open();
        qry1 = "select *,s1.Bank_Name,FORMAT(rak_form_submit_dt,'dd/MM/yyyy') as com_dt,s2.discription from mem_bank_rakyat left join Ref_Nama_Bank s1 on s1.Bank_Code=rak_bank_cd left join ref_appl_bank_sts s2 on s2.code=rak_appl_sts_cd where Acc_sts='Y'";
    ...
    {
        Response.Redirect("../keanggotan/Kmk_anggota.aspx");
    }
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ddokdicno = DBCon.Ora_Execute_table("select * from mem_bank_rakyat where rak_new_icno='" + lblTitle.Text + "' ");
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../keanggotan/Kmk_anggota.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
