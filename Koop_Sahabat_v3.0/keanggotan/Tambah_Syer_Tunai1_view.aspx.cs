...
public partial class Tambah_Syer_Tunai1_view : System.Web.UI.Page
{
    ...
    {
        con.Open();
        qry1 = "select mem_new_icno,mem_name,mem_centre,case when mem_sts_cd = 'SA' then 'ANGGOTA SAH' when mem_sts_cd = 'TS' then 'ANGGOTA TIDAK SAH' when mem_sts_cd = 'FM' then 'FI MASUK' else 'DALAM NOTIS' end as mm_sts from mem_member where Acc_sts ='Y' and mem_sts_cd IN ('SA','FM')";
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../keanggotan/Tambah_Syer_Tunai1.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
