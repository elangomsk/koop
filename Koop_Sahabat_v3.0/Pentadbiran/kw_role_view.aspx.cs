...
public partial class kw_role_view : System.Web.UI.Page
{
    ...
    protected void BindData()
    {
        ...
        {
            qry1 = "select *, case when Status = 'A' then 'AKTIF' else 'TIDAK AKTIF' end as sts from KK_PID_Kumpulan order by KK_kumpulan_id";
        }
        else
        {
            qry1 = "select *, case when Status = 'A' then 'AKTIF' else 'TIDAK AKTIF' end as sts from KK_PID_Kumpulan where KK_kumpulan_id NOT IN ('R0004') order by KK_kumpulan_id";
        }
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ddokdicno = Dblog.Ora_Execute_table("select * from KK_PID_Kumpulan where ID='" + ogid + "'");
            if (ddokdicno.Rows.Count != 0)
            {
                string name = HttpUtility.UrlEncode(service.Encrypt(og_genid.Text));
                Response.Redirect(string.Format("../Pentadbiran/kw_role.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
