...
public partial class kw_user_registration_view : System.Web.UI.Page
{
    ...
    void get_filter_info()
    {
        if (DropDownList1.SelectedValue == "N")
        {
            ...
            {
                sqry = "and (KK_username like '%" + txt_srch.Text + "%' OR KK_userid like '%" + txt_srch.Text + "%')";
            }
        }
        else
        {
    ...
    {
        get_filter_info();
        qry1 = "select *,case when Status = 'A' then 'AKTIF' else 'TIDAK AKTIF' end as sts,case when ISNULL(user_img,'') = '' then 'user.gif' else user_img end as img1  from KK_User_Login where KK_user_type='"+ DropDownList1.SelectedValue + "' "+ sqry + " order by KK_crt_dt,ID";
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ddokdicno = Dblog.Ora_Execute_table("select * from KK_User_Login where ID='" + ogid + "'");
            if (ddokdicno.Rows.Count != 0)
            {
                string name = HttpUtility.UrlEncode(service.Encrypt(og_genid.Text));
                Response.Redirect(string.Format("../Pentadbiran/kw_user_registration.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
