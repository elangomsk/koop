...
public partial class kw_projek_view : System.Web.UI.Page
{
...

    {
        if (Session["New"] != null)
        {
            ...
            gt_lng = DBCon.Ora_Execute_table("select " + Session["site_languaage"].ToString() + " from Ref_language where ID IN ('169','705','133','39')");
    ...
    protected void BindData()
    {
        ...
        //else
        //{
        //    sqry = "where s1.Ref_Projek_code LIKE'%" + txtSearch.Text + "%' OR s1.Ref_Projek_name LIKE'%" + txtSearch.Text + "%'";
        //}
        con.Open();
        DataTable ddicno = new DataTable();
        SqlCommand cmd = new SqlCommand("select s1.id,s1.Ref_Projek_code,s1.Ref_Projek_name,s1.Ref_Projek_descript,case when s1.Status='A' then 'AKTIF' else 'TIDAK AKTIF' end as sts,FORMAT(s1.cr_dt,'dd/MM/yyyy', 'en-us') as dt1 from KW_Ref_Projek s1 " + sqry + " order by s1.Ref_Projek_code asc", con);
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../kewengan/kw_projek.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
