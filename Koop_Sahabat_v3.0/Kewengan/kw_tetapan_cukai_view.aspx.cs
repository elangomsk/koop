...
public partial class kw_tetapan_cukai_view : System.Web.UI.Page
{
    ...
    protected void BindData()
    {
        ...
        SqlCommand cmd = new SqlCommand("select s1.id,s1.Ref_nama_cukai,s1.Ref_kod_cukai,s1.Ref_kadar,s1.Ref_kod_akaun,case when s1.tc_jenis = 'PEN' then 'PENERIMAAN' when s1.tc_jenis = 'PEM' then 'PEMBAYARAN' else 'OTHERS' end as jenis,s2.Ref_Jenis_cukai,s3.nama_akaun,s1.Ref_tarikh_mula,s1.Ref_tarikh_akhir,s1.Ref_klasi_akaun,s1.Ref_descripsi,case when s1.Status='A' then 'AKTIF' else 'TIDAK AKTIF' end as sts from KW_Ref_Tetapan_cukai s1 left join KW_Ref_Jenis_Cukai s2 on s2.Id=s1.Ref_jenis_cukai left join KW_Ref_Carta_Akaun s3 on s3.kod_akaun=s1.Ref_kod_akaun " + sqry + " order by s1.ID desc", con);
        ...
        if (rcount != "0")
        {
            foreach (GridViewRow row in gv_refdata.Rows)
            {
                ...
                if (rbn.Checked)
                {
                    ...
                    SqlCommand ins_peng = new SqlCommand("Delete from KW_Ref_Tetapan_cukai where id='" + varName1 + "'", conn);
                    conn.Open();
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../kewengan/kw_tetapan_cukai.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
