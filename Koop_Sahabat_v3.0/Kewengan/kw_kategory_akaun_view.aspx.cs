...
public partial class kw_kategory_akaun_view : System.Web.UI.Page
{
...

    {
        if (Session["New"] != null)
        {
            ...
            gt_lng = DBCon.Ora_Execute_table("select " + Session["site_languaage"].ToString() + " from Ref_language where ID IN ('711','705','39')");
    ...
    protected void BindData()
    {
        ...
        //}
        qry1 = "select Id,kat_cd,kat_akuan,deskripsi,bal_type,case when bal_type='K' then 'KREDIT' when bal_type='D' then 'DEBIT' else '' end btype,case when Status='A' then 'AKTIF' else 'TIDAK AKTIF' end as sts,case when kat_type='01' then 'CARTA AKAUN' else 'BAJET' end as jen_kat From KW_Kategori_akaun " + sqry + " order by kat_type,kat_cd asc";
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../kewengan/kw_kategory_akaun.aspx?edit={0}", name));
            }
            else
            {
