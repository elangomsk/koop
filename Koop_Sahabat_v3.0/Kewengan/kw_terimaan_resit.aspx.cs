...
public partial class kw_terimaan_resit : System.Web.UI.Page
{
    ...
    void bind_akaun_bank()
    {
        ...
        try
        {
            ...
            string com = "SELECT Ref_kod_akaun, Ref_Nama_akaun FROM KW_Ref_Akaun_bank where status='A'";
            SqlDataAdapter adpt = new SqlDataAdapter(com, con);
            DataTable dt = new DataTable();
            adpt.Fill(dt);
    ...
    void bind_invois()
    {
        ...
        try
        {
            ...
            string com = "select * from (select a.no_invois,case when a.overall=b.overall then 'TRUE' else 'FALSE' end as sts from (select no_invois,Overall from KW_Penerimaan_invois where nama_pelanggan_code='"+ dd_kepada.SelectedValue +"' and ISNULL(kew_appr_gl_cd,'')='L') as a outer apply (select no_invois,Overall from KW_Penerimaan_resit where ISNULL(gl_sts,'')='L' and a.no_invois=no_invois) as b ) as a1 where a1.sts='FALSE'";
            SqlDataAdapter adpt = new SqlDataAdapter(com, con);
            DataTable dt = new DataTable();
            adpt.Fill(dt);
    ...
    void bind_invois1()
    {
        ...
        try
        {
            ...
            string com = "select no_invois from KW_Penerimaan_invois where nama_pelanggan_code='" + dd_kepada.SelectedValue + "' and ISNULL(kew_appr_gl_cd,'')='L'";
            SqlDataAdapter adpt = new SqlDataAdapter(com, con);
            DataTable dt = new DataTable();
            adpt.Fill(dt);
    ...
    void bind_project()
    {
        ...
        try
        {
            string com = "SELECT s2.Ref_Projek_code,Ref_Projek_name FROM KW_Penerimaan_invois s1 left join KW_Ref_Projek s2 on s2.Ref_Projek_code=s1.project_kod WHERE nama_pelanggan_code='" + dd_kepada.SelectedValue + "' and ISNULL(s2.Ref_Projek_code,'') != '' group by s2.Ref_Projek_code,Ref_Projek_name";
            SqlDataAdapter adpt = new SqlDataAdapter(com, con);
            DataTable dt = new DataTable();
            adpt.Fill(dt);
    ...
    {
        if(DropDownList5.SelectedItem.Text.ToUpper() == "TUNAI")
        {
            mak_resit.Visible = false;
        }
        else
        {
            mak_resit.Visible = true;
        }
    }
    ...
    protected void get_invois_info(object sender, EventArgs e)
    {
        get_invois();
    }
    ...
    void get_invois()
    {
        if (dd_terma.SelectedValue != "")
        {
            DataTable dt2 = new DataTable();
            dt2 = Dbcon.Ora_Execute_table("select Ref_doc_name jp,unit,discount,quantiti,s3.Ref_kod_cukai as gsttype,s3.Ref_nama_cukai as gstname,keterangan,CAST(CASE c.tax WHEN 'Y' THEN 1 ELSE 0 END AS BIT) AS tax,tax othgst,case when c.gstjumlah = '0.00' then '0.00' else Format(c.gstjumlah,'#,##,###.00') end as gstjumlah , '0.00' as othgstjumlah , case when c.jumlah = '0.00' then '0.00' else Format(c.jumlah,'#,##,###.00') end as jumlah , case when c.overall = '0.00' then '0.00' else Format(c.overall,'#,##,###.00') end as overall,c1.perkera,c1.project_kod,s4.Ref_doc_code   from KW_Penerimaan_invois_item c left join KW_Penerimaan_invois c1 on c1.no_invois=c.no_invois left join KW_Ref_Tetapan_cukai s3 on s3.Ref_kod_cukai=c.tax left join KW_Ref_Doc_kod s4 on s4.Ref_doc_code='09' and s4.Status='A' where c1.no_invois='" + dd_terma.SelectedValue + "' "
                + " union all  select Ref_doc_name jp,res_unit unit, res_disk discount,res_quantiti quantiti,s3.Ref_kod_cukai as gsttype,s3.Ref_nama_cukai as gstname,res_keterangan,CAST(CASE c.tax WHEN 'Y' THEN 1 ELSE 0 END   AS BIT) AS tax,tax othgst,case when c.gstjumlah = '0.00' then '0.00' else Format(c.gstjumlah,'#,##,###.00') end as gstjumlah , '0.00' as othgstjumlah , case when c.res_jumlah = '0.00' then   '0.00' else Format(c.res_jumlah,'#,##,###.00') end as jumlah , case when c.overall = '0.00' then '0.00' else Format(c.overall,'#,##,###.00') end as overall,c1.perkara perkera,c1.project_kod,s4.Ref_doc_code   from KW_Penerimaan_resit_item c left join KW_Penerimaan_resit c1 on c1.no_resit=c.no_resit left join KW_Ref_Tetapan_cukai s3 on s3.Ref_kod_cukai=c.tax left join KW_Ref_Doc_kod s4 on s4.Ref_doc_code='02' and s4.Status='A'   where c1.no_invois='" + dd_terma.SelectedValue + "' and c.no_resit !='"+ txtnoper.Text + "' "
                + "union all  select Ref_doc_name jp,unit,discount,quantiti,s3.Ref_kod_cukai as gsttype,s3.Ref_nama_cukai as gstname,keterangan,CAST(CASE c.tax WHEN 'Y' THEN 1 ELSE 0 END AS BIT) AS tax,othgst,case when c.gstjumlah = '0.00' then '0.00' else Format(c.gstjumlah,'#,##,###.00') end as gstjumlah , '0.00' as othgstjumlah , case when c.jumlah = '0.00' then '0.00' else Format(c.jumlah,'#,##,###.00') end as jumlah , case when c.overall = '0.00' then '0.00' else Format(c.overall,'#,##,###.00') end as overall,c1.perkara perkera,c1.project_kod,s4.Ref_doc_code from KW_Penerimaan_Credit_item c left join KW_Penerimaan_Credit c1 on c1.no_notakredit=c.no_notakredit left join KW_Ref_Tetapan_cukai s3 on s3.Ref_kod_cukai=c.tax left join KW_Ref_Doc_kod s4 on s4.Ref_doc_code='10' and s4.Status='A' where c1.no_invois='" + dd_terma.SelectedValue + "' "
                + "union all  select Ref_doc_name jp,unit,discount,quantiti,s3.Ref_kod_cukai as gsttype,s3.Ref_nama_cukai as gstname,keterangan,CAST(CASE c.tax WHEN 'Y' THEN 1 ELSE 0 END AS BIT) AS tax,othgst,case when c.gstjumlah = '0.00' then '0.00' else Format(c.gstjumlah,'#,##,###.00') end as gstjumlah , '0.00' as othgstjumlah , case when c.jumlah = '0.00' then '0.00' else Format(c.jumlah,'#,##,###.00') end as jumlah , case when c.overall = '0.00' then '0.00' else Format(c.overall,'#,##,###.00') end as overall,c1.perkara perkera,c1.project_kod,s4.Ref_doc_code from KW_Penerimaan_Debit_item c left join KW_Penerimaan_Debit c1 on c1.no_notadebit=c.no_notadebit left join KW_Ref_Tetapan_cukai s3 on s3.Ref_kod_cukai=c.tax left join KW_Ref_Doc_kod s4 on s4.Ref_doc_code='11' and s4.Status='A' where c1.no_invois='" + dd_terma.SelectedValue + "'");
            Gridview1.DataSource = dt2;
            Gridview1.DataBind();
            //Gridview1.FooterRow.Cells[2].HorizontalAlign = HorizontalAlign.Right;
            //((Label)Gridview1.FooterRow.Cells[12].FindControl("jumlah_ftr")).Text = jum_unit.ToString("C").Replace("RM", "").Replace("$", "");
            if (dt2.Rows.Count != 0)
            {
                Gridview1.Visible = true;
                ddbkepada.SelectedValue = dt2.Rows[0]["project_kod"].ToString();
                TextBox16.Text = dt2.Rows[0]["perkera"].ToString();
                TextBox23.Text = dt2.Rows[0]["keterangan"].ToString();
    ...
    protected void Button2_Click(object sender, EventArgs e)
    {
        ...
        //{
            ...
            string cek_dt = string.Empty;
                if (DropDownList5.SelectedValue != "" && DropDownList4.SelectedValue != "" && dd_kepada.SelectedValue != "" && TextBox7.Text != "" && TextBox16.Text != "")
                {
                ...
                userid = Session["New"].ToString();
                                    ...
                                    string Inssql_main = "insert into KW_Penerimaan_resit (tarikh_resit,no_resit,project_kod,no_invois,bentuk_bayaran,nama_pelanggan_code,perkara,gl_cd_bank,no_cek,bank_cod,tarikh_cek,Overall,lulus_sts,crt_id,cr_dt,Status) values('" + doc_dt + "','" + txtnoper_1.Text + "','" + ddbkepada.SelectedValue + "','"+ dd_terma.SelectedValue +"','" + DropDownList5.SelectedValue + "','" + dd_kepada.SelectedValue + "','" + TextBox16.Text + "','" + DropDownList4.SelectedValue + "','" + TextBox19.Text + "','" + DropDownList2.SelectedValue + "','" + cek_dt + "','" + TextBox17.Text+ "','L','" + userid + "','" + DateTime.Now.ToString("yyyy/MM/dd hh:mm:ss") + "','A')";
    ...
    protected void btnkem_Click(object sender, EventArgs e)
    {
        if (txtnoper.Text != "")
        {
            if (DropDownList5.SelectedValue != "" && DropDownList4.SelectedValue != "" && dd_kepada.SelectedValue != "" && TextBox7.Text != "")
            {
                ...
                userid = Session["New"].ToString();
                DataTable dt = new DataTable();
                string Inssql_main = "update KW_Penerimaan_resit  set project_kod='"+ ddbkepada.SelectedValue + "',no_invois='" + dd_terma.SelectedValue + "',bentuk_bayaran='" + DropDownList5.SelectedValue + "',nama_pelanggan_code='" + dd_kepada.SelectedValue + "',perkara='" + TextBox16.Text + "',gl_cd_bank='" + DropDownList4.SelectedValue + "',no_cek='" + TextBox19.Text + "',bank_cod='" + DropDownList2.SelectedValue + "',tarikh_cek='" + cek_dt + "',upd_id='" + userid + "',upd_dt='" + DateTime.Now.ToString("yyyy/MM/dd HH:mm:ss") + "' where no_resit='" + txtnoper.Text + "'";
                Status = DBCon.Ora_Execute_CommamdText(Inssql_main);
                if (Status == "SUCCESS")
                {
                    BindMohon();
                    ...
                    reset();
                    tab1();
                    ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "$.Zebra_Dialog('Terimaan Berjaya Dikemaskini.',{'type': 'confirmation','title': 'Success','auto_close': 2000});", true);
                }
        ...
        {
            ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "$.Zebra_Dialog('Sila Masukkan No Invois.',{'type': 'warning','title': 'warning','auto_close': 2000});", true);
        }
    }
    ...
    void crt_invois()
    {
        ...
        DataTable dt = new DataTable();
        dt = Dbcon.Ora_Execute_table("select no_resit,bentuk_bayaran,no_cek,gl_cd_bank,no_invois,lulus_sts,gl_sts,bank_cod,project_kod,nama_pelanggan_code as bayar_kepada,Status,Overall as jumlah,Format(tarikh_resit,'dd/MM/yyyy') as tk_invois,Format(tarikh_cek,'dd/MM/yyyy') as tk_cek,'' as tk_mohon,'' cara_bayaran,'' kat_penerima,perkara from KW_Penerimaan_resit where no_resit='" + ssv2 + "' and Status='A'");
        //tab1();
        if (dt.Rows.Count != 0)
        {
            if (dt.Rows[0]["gl_sts"].ToString() == "L")
            {
                kem_Button2.Visible = false;
            }
            else
            ...
            Session["dbtno"] = dt.Rows[0]["no_resit"].ToString();
            Button23.Visible = false;
            Button24.Visible = false;
            //TextBox3.Text = dt.Rows[0]["ID_pemohon"].ToString();
            txttarkihmo.Text = dt.Rows[0]["tk_mohon"].ToString();
            TextBox16.Text = dt.Rows[0]["perkara"].ToString();
            
            //txtname.Text = dt.Rows[0]["no_ruj"].ToString();
            txtnoper.Text = dt.Rows[0]["no_resit"].ToString();
            txtnoper.Attributes.Add("Readonly", "Readonly");
            DropDownList2.SelectedValue = dt.Rows[0]["bank_cod"].ToString();
            DropDownList1.SelectedValue = "03";
            shw_grd_info();
            dd_kepada.SelectedValue = dt.Rows[0]["bayar_kepada"].ToString();
            bind_project();
            bind_invois1();
            DropDownList5.SelectedValue = dt.Rows[0]["bentuk_bayaran"].ToString();
            if(DropDownList5.SelectedItem.Text.ToUpper() == "TUNAI")
            {
                mak_resit.Visible = false;
            }
            ...
            if (dt.Rows[0]["gl_cd_bank"].ToString() != "")
            {
                DropDownList4.SelectedValue = dt.Rows[0]["gl_cd_bank"].ToString();
            }
            ddbkepada.SelectedValue = dt.Rows[0]["project_kod"].ToString();
            dd_terma.SelectedValue = dt.Rows[0]["no_invois"].ToString();
            TextBox7.Text = dt.Rows[0]["tk_invois"].ToString();
            TextBox19.Text = dt.Rows[0]["no_cek"].ToString();
            TextBox22.Text = dt.Rows[0]["tk_cek"].ToString();
          
            
            query1 = "select UPPER(res_keterangan) as Keteragan,'' as bank,'' as acc_no,res_unit as jumlah, res_disk as Disk,res_quantiti as qty, "
                           + " UPPER(ISNULL(s4.Ref_nama_cukai,'')) as jen_cukai, s1.gstjumlah as cukai_amt, s1.Overall as amount,res_jumlah as tot  from KW_Penerimaan_resit_item s1  "
                           + " left join KW_Ref_Tetapan_cukai s4 on s4.Ref_kod_cukai=s1.tax where no_resit='" + ssv2 + "' and s1.Status='A'";
    ...
    protected void BindMohon()
    {
        ...
        qry1 = "select '03' kat_penerima,Format(tarikh_resit,'dd/MM/yyyy') tarkih_permohonan,a.no_resit no_invois,nama_pelanggan_code as  Bayar_kepada,a.Overall as  jumlah,CASE ISNULL(a.lulus_sts,'')  WHEN '' THEN 'WAITING'  WHEN 'P' THEN 'PROSES' WHEN 'L' THEN 'LULUS' END  as status,ISNULL(gl_sts,'') lul_sts_cd,CASE ISNULL(gl_sts,'')  WHEN '' THEN 'WAITING'  WHEN 'P' THEN 'PROSES' WHEN 'L' THEN 'LULUS' END  as lul_status,a.no_invois po_no,a.perkara as keterangan  from KW_Penerimaan_resit a left join KW_Ref_Pelanggan s1 on s1.Ref_no_syarikat=a.nama_pelanggan_code  where a.Status='A' " + condtion + "";
    ...
    protected void Gridview12_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            ddlCountries1.DataSource = GetData("select Ref_nama_cukai,Ref_kod_cukai from KW_Ref_Tetapan_cukai where tc_jenis='PEN'");
    ...
    {
        DataTable dt1 = Dbcon.Ora_Execute_table("select doc_type_cd,format as fmt,SUBSTRING(cur_format, 1, 12) as lfrmt1,SUBSTRING(cur_format, 13, 4) as lfrmt2,cur_format as cfmt from KW_Format_Nombor_rujukan where doc_type_cd='02' and Status='A'");
        if (dt1.Rows.Count != 0)
        {
            if (dt1.Rows[0]["cfmt"].ToString() == "")
            {
        ...
        {
            DataTable get_doc = new DataTable();
            get_doc = Dbcon.Ora_Execute_table("select Ref_doc_descript as s1,s1.ws_format as s2 from KW_Ref_Doc_kod left join site_settings s1 on  s1.ID='1' where Ref_doc_code='02'");
            DataTable dt = Dbcon.Ora_Execute_table("select   ISNULL(max(SUBSTRING(no_resit,13,3000)),'0') from KW_Penerimaan_resit");
            if (dt.Rows.Count > 0)
            {
                int seqno = Convert.ToInt32(dt.Rows[0][0].ToString());
                int newNumber = seqno + 1;
    ...
    protected void grdbilinv_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            ddlCountries2.DataSource = GetData("select Ref_nama_cukai,Ref_kod_cukai from KW_Ref_Tetapan_cukai where tc_jenis='PEN'");
            ddlCountries2.DataTextField = "Ref_nama_cukai";
            ddlCountries2.DataValueField = "Ref_kod_cukai";
            ddlCountries2.DataBind();
    ...
    protected void grdinvois_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            ddlCountries1.DataSource = GetData("select Ref_nama_cukai,Ref_kadar from KW_Ref_Tetapan_cukai where tc_jenis='PEN'");
            ddlCountries1.DataTextField = "Ref_nama_cukai";
    ...
    protected void Gridview3_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            System.Web.UI.WebControls.Label amn = (System.Web.UI.WebControls.Label)e.Row.FindControl("lblover");
            System.Web.UI.WebControls.Label type_cd = (System.Web.UI.WebControls.Label)e.Row.FindControl("Label22");
            if (type_cd.Text == "09")
            {
                total += float.Parse(amn.Text);
            }
        ...
        {
            Label jum_ftr = (e.Row.FindControl("jumlah_ftr") as Label);

            jum_ftr.Text = total.ToString("C").Replace("$", "").Replace("RM", "");
    ...
    protected void grdmohon_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            System.Web.UI.WebControls.TextBox ket_txt = (System.Web.UI.WebControls.TextBox)e.Row.FindControl("TextBox13");
            ket_txt.Text = "PENERIMAAN "+ TextBox23.Text;
    ...
    void shw_grd_info()
    {
        ...
        if (DropDownList1.SelectedValue != "")
        {
            ...
            if (DropDownList1.SelectedValue == "01")
            {
            dd_kepada.DataSource = GetData("SELECT stf_name as val1,stf_staff_no as val2 FROM hr_staff_profile s1 left join hr_post_his s2 on s2.pos_staff_no=s1.stf_staff_no and pos_end_dt >='" + DateTime.Now.ToString("yyyy-MM-dd") + "' where ISNULL(stf_kod_akaun,'') != ''");
            }
            else if (DropDownList1.SelectedValue == "02")
            {
            dd_kepada.DataSource = GetData("select Ref_nama_syarikat as val1,Ref_no_syarikat as val2 from KW_Ref_Pembekal where Status='A' and ISNULL(Ref_kod_akaun,'') != ''");
            }
            else if (DropDownList1.SelectedValue == "03")
            {
            dd_kepada.DataSource = GetData("select Ref_nama_syarikat as val1,Ref_no_syarikat as val2 from KW_Ref_Pelanggan where Status='A' and ISNULL(Ref_kod_akaun,'') != ''");
            }
        dd_kepada.DataTextField = "val1";
        dd_kepada.DataValueField = "val2";
        dd_kepada.DataBind();

        //Add Default Item in the DropDownList
        dd_kepada.Items.Insert(0, new ListItem("--- PILIH ---", ""));
