...
public partial class kw_lap_bajet : System.Web.UI.Page
{
    ...
    protected void BindGrid()
    {
        ...
        string query = " with Recurse as ("
                          ...
                          + " select * from (select a.DirectChildId, isnull(sum(cast(Ref_jumlah_bajet as money)),'0.00')  as Amount "
                            ...
                            + " (isnull(sum(cast(Ref_jumlah_bajet as money)),'0.00') - isnull(sum(cast(ref_shadow_bajet as money)),'0.00')) baki_payang "
    ...
    protected void clk_update(object sender, EventArgs e)
    {
        ...
        if (get_value.Rows.Count != 0)
        {
            ...
            sel_val3 = DBCon.Ora_Execute_table("select *,Format(Ref_tk_mula,'dd/MM/yyyy') s1,Format(Ref_tk_akhir,'dd/MM/yyyy') s2 From KW_Ref_Bajet where Ref_kod_bajet='" + get_value.Rows[0]["kod_bajet"].ToString() + "' and Ref_kat_bajet='" + lbl1.Text + "' and ref_bjt_year ='" + tah_kewangan.SelectedValue + "' and Status='A'");
    ...
    protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        System.Web.UI.WebControls.Label clmn8 = (System.Web.UI.WebControls.Label)e.Row.FindControl("Label3");
        System.Web.UI.WebControls.Label clmn5 = (System.Web.UI.WebControls.Label)e.Row.FindControl("Label3_1");
        System.Web.UI.WebControls.Label clmn6 = (System.Web.UI.WebControls.Label)e.Row.FindControl("Label3_2");
        System.Web.UI.WebControls.Label clmn7 = (System.Web.UI.WebControls.Label)e.Row.FindControl("Label3_3");
        LinkButton Button3 = e.Row.FindControl("LinkButton1") as LinkButton;

        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            if (clmn6.Text != "0.00" || clmn8.Text != "0.00")
            {
                float ss1 = ((float.Parse(clmn6.Text) / float.Parse(clmn8.Text)) * 100);
                clmn7.Text = ss1.ToString();
            }
            else
            {
                clmn7.Text = "0";
    ...
    protected void clk_submit(object sender, EventArgs e)
    {
        ...
        if (TextBox1.Text != "" && dd_kodind.SelectedValue != "" && TextBox4.Text != "")
        {
            ...
            if (sel_val.Rows.Count != 0)
            {
                    ...
                    DataTable sel_val3 = new DataTable();
                    sel_val3 = DBCon.Ora_Execute_table("select * From KW_Ref_Bajet where Ref_kod_bajet='" + sel_val.Rows[0]["kod_bajet"].ToString() + "' and Ref_kat_bajet='" + dd_kodind.SelectedValue + "' and Ref_tk_mula='" + fmdate + "' and Ref_tk_akhir='" + tmdate + "' and Status='A'");
                    if (sel_val3.Rows.Count == 0)
                    {
                        string Inssql1 = "Insert into KW_Ref_Bajet (Ref_kod_bajet,Ref_kat_bajet,Ref_jumlah_bajet,Ref_tk_mula,Ref_tk_akhir,Status,Ref_nota_bajet,crt_id,cr_dt) values('" + sel_val.Rows[0]["kod_bajet"].ToString() + "','" + dd_kodind.SelectedValue + "','" + double.Parse(TextBox4.Text).ToString("C").Replace("RM", "").Replace("$", "") + "','" + fmdate + "','" + tmdate + "','A','"+ txt_nota.Value +"','" + Session["New"].ToString() + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "')";
