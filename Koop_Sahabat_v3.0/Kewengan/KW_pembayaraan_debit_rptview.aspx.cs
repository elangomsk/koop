...
public partial class KW_pembayaraan_debit_rptview : System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                ...
                if (nameshbt != "")
                {
                    ...
                    dt = Dbcon.Ora_Execute_table("select distinct no_invois,untuk_akaun,ss1.nama_akaun as nama_syarikat,ISNULL(ss2.Ref_alamat,'') Ref_alamat,ISNULL(ss2.ref_alamat_ked,'') ref_alamat_ked,ISNULL(ss2.pel_bandar,'') as pem_bandar,ISNULL(n1.hr_negeri_desc,'') hr_negeri_desc,ISNULL(ss2.pel_poskod,'') as pem_poskod,ISNULL(c1.CountryName,'') CountryName,ISNULL(keterangan,'') keterangan,no_Rujukan,jumlah,FORMAT(ISNULL(tarikh_credit,''),'dd/MM/yyyy', 'en-us') as tarikh_credit from KW_Pembayaran_Dedit_item left join KW_Ref_Carta_Akaun ss1 on ss1.kod_akaun=nama_pelanggan left join KW_Ref_Pelanggan ss2 on ss2.Ref_kod_akaun=ss1.kod_akaun left join Country c1 on c1.ID=ss2.pel_negera left join Ref_hr_negeri n1 on n1.hr_negeri_Code=ss2.pel_negeri where no_Rujukan='" + nameshbt + "' ");
                    ...
                    if (countRow != 0)
                    {
                        ...
                        //Path
                        rptdebit.LocalReport.ReportPath = "kewengan/KW_pem_Debit_Note.rdlc";
                        string branch;
