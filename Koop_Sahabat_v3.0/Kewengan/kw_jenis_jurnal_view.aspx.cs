...
public partial class kw_jenis_jurnal_view : System.Web.UI.Page
{
    ...
    protected void BindData()
    {
        ...
        //}
        qry1 = "select Id,jur_desc,jur_type_cd,status,Jur_item,REPLACE(REPLACE(Jur_item,'D','Debit'),'K','Kerdit') as item From KW_ref_jurnal_type   " + sqry + "  order by jur_type_cd asc";
        ...
        if (rcount != "0")
        {
            foreach (GridViewRow row in gv_refdata.Rows)
            {
                ...
                if (rbn.Checked)
                {
                    ...
                    SqlCommand ins_peng = new SqlCommand("Delete from KW_ref_jurnal_type where id='" + varName1 + "'", conn);
                    conn.Open();
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../kewengan/kw_jenis_jurnal.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
