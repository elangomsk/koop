...
public partial class kw_template_aliran : System.Web.UI.Page
{
    ...
    string tn1 = string.Empty, tc1 = string.Empty, tc2 = string.Empty, tc3 = string.Empty, tc4 = string.Empty, tc5 = string.Empty, tc6 = string.Empty, tc7 = string.Empty, tc8 = string.Empty, tc9 = string.Empty;
    ...
    protected void gv_refdata_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            if ((e.Row.RowState & DataControlRowState.Edit) > 0)
            {
                ...
                ddList.DataTextField = "ref_alrn_nama";
                ...
                ddList1.DataTextField = "ref_alrn_nama";
        ...
        if (e.Row.RowType == DataControlRowType.Footer)
        {
            ...
            lbladdwilayah.DataTextField = "ref_alrn_nama";
            ...
            lbladdwilayah1.DataTextField = "ref_alrn_nama";
    ...
    public DataTable load_department()
    {
        ...
        sqlcon.Open();
        string sql = "SELECT Id,UPPER(ref_alrn_nama) ref_alrn_nama FROM KW_Ref_aliran_item WHERE ref_alrn_header='K'";
    ...
    public DataTable load_department1()
    {
        ...
        SqlConnection sqlcon = new SqlConnection(cs);
        sqlcon.Open();
        string sql = "SELECT Id,UPPER(ref_alrn_nama) ref_alrn_nama FROM KW_Ref_aliran_item WHERE under_parent='" + Session["get_cd"].ToString() + "' and ref_alrn_header='Y'";
        SqlCommand cmd = new SqlCommand(sql);
        cmd.CommandType = CommandType.Text;
        cmd.Connection = sqlcon;
        SqlDataAdapter sd = new SqlDataAdapter(cmd);
    ...
    protected void BindData()
    {
        ...
        {
            qry1 = "Select s2." + tc1 + " as c1,s2." + tc2 + " as c2,s2." + tc3 + " as c3,s3.ref_alrn_nama as c5,s2." + tc6 + " as c6,s2." + tc4 + " as c4,s3.Id as c7,case when s2.ref_alrn_header='Y' then s3.ref_alrn_nama else s4.ref_alrn_nama end as c8,case when s2.ref_alrn_header='Y' then s3.Id else s4.Id end as c9 from " + tn1 + " s2 left join KW_Ref_aliran_item s3 on s3.Id=s2.under_parent and s3.ref_alrn_header = '" + tc8 + "' left join KW_Ref_aliran_item s4 on s4.Id=s3.under_parent and s4.ref_alrn_header = '" + tc9 + "'  where s2." + tc6+" = '"+ tc7 + "'  ORDER BY s2." + tc1 + " ASC";
        }
        else
        {
            qry1 = "select kavasan_name as c1,wilayah_code as c2,wilayah_name as c3,cawangan_code as c4,'' as c5,'' as c6,'' as c7,'' as c8,'' as c9 from ref_cawangan where kawasan_code = '00000'";
        }
    ...
    protected void gv_refdata_RowUpdating(object sender, GridViewUpdateEventArgs e)
    {
        ...
        if (dtcenter.Rows.Count == 0)
        {
            ...
            string Inssql = "update " + tn1 + " set " + tc1 + "='" + lblEditName.Text + "'," + tc2 + "='" + txtEditcode.Text + "'," + tc3 + "='" + editddlStatus.SelectedValue + "'," + tc5 + "='" + up_val + "',upd_id='" + Session["new"].ToString() + "',upd_dt='"+ DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") +"' where " + tc4 + "='" + Id.Text + "'";
            Status = DBCon.Ora_Execute_CommamdText(Inssql);
            if (Status == "SUCCESS")
            {
                Session["get_cd"] = "";
