...
public partial class kw_pembekal_view : System.Web.UI.Page
{
...

    {
        if (Session["New"] != null)
        {
            ...
            gt_lng = DBCon.Ora_Execute_table("select " + Session["site_languaage"].ToString() + " from Ref_language where ID IN ('740','705','133','39')");
    ...
    protected void BindData()
    {
        ...
        //else
        //{
        //    sqry = "where s1.Ref_nama_syarikat LIKE'%" + txtSearch.Text + "%' OR s1.Ref_email LIKE'%" + txtSearch.Text + "%'";
        //}
        con.Open();
        DataTable ddicno = new DataTable();
        SqlCommand cmd = new SqlCommand("select s1.id,s1.Ref_nama_syarikat,s1.Ref_no_syarikat,s1.Ref_no_telefone,s1.Ref_no_fax,s1.Ref_email,s1.Ref_gst_id,case when s1.Status='A' then 'AKTIF' else 'TIDAK AKTIF' end as sts from KW_Ref_Pembekal s1 " + sqry + " order by s1.ID desc", con);
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../kewengan/kw_pembekal.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
        ...
        if (rcount != "0")
        {
            foreach (GridViewRow row in gv_refdata.Rows)
            {
                ...
                if (rbn.Checked)
                {
                    ...
                    DataTable get_kod = new DataTable();
                    get_kod = DBCon.Ora_Execute_table("select * From KW_Ref_Pembekal where Id='" + varName1 + "' ");
                    Session["pem_id"] = get_kod.Rows[0]["Ref_kod_akaun"].ToString();
                    string Inssql = "delete from KW_Ref_Pembekal where Id = '" + varName1 + "'";
                    Status = DBCon.Ora_Execute_CommamdText(Inssql);

                    if (Status == "SUCCESS")
                    {
                        string Inssql1 = "delete from KW_Ref_Carta_Akaun where kod_akaun = '" + Session["pem_id"].ToString() + "'";
