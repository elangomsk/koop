...
public partial class kw_jenis_cukai_view : System.Web.UI.Page
{
    ...
    protected void BindData_jenis()
    {
        ...
        SqlCommand cmd = new SqlCommand("select s2.Id,s2.Ref_Jenis_cukai,case when s2.Status='A' then 'AKTIF' else 'TIDAK AKTIF' end as sts from KW_Ref_Jenis_Cukai s2 "+ sqry + " order by s2.Ref_Jenis_cukai asc", con);
        ...
        if (rcount != "0")
        {
            foreach (GridViewRow row in GridView1.Rows)
            {
                ...
                if (rbn.Checked)
                {
                    ...
                    SqlCommand ins_peng = new SqlCommand("Delete from KW_Ref_Jenis_Cukai where id='" + varName1 + "'", conn);
                    conn.Open();
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ddokdicno = DBCon.Ora_Execute_table("select * From KW_Ref_Jenis_Cukai where Id='" + lblTitle.Text + "' ");
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../kewengan/kw_jenis_cukai.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
