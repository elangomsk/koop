...
public partial class kw_pembayaran_kredit : System.Web.UI.Page
{
        ...
        if (tOt.ToString() != "")
        {
            ...
            if (tgst_1 != "--- PILIH ---")
            {
                if (chk.Checked == true)
                {
                    ...
                    decimal tot1 = (Convert.ToDecimal(tOt.Text) - Convert.ToDecimal(dis1));
                    ...
                    numTotal3 = Convert.ToDecimal(tOt.Text);
                ...
                else
                {
                    ...
                    if (dis.Text != "")
                    {
                        numTotal2 = (Convert.ToDecimal(tOt.Text) - Convert.ToDecimal(dis.Text)) + numTotal1 + Convert.ToDecimal(gmt.Text);
                    }
                    else
                    {
            ...
            else
            {
                ...
                if (dis.Text != "")
                {
                    numTotal1 = (Convert.ToDecimal(tOt.Text) - Convert.ToDecimal(dis.Text));
                }
                else
                {
                    numTotal1 = (Convert.ToDecimal(tOt.Text));
                }
                if (chk.Checked == true)
                {
                    if (gmt.Text != "")
        ...
        if (ViewState["CurrentTable3"] != null)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    TextBox box2_new = (TextBox)Gridview10.Rows[rowIndex].Cells[4].FindControl("Txtdis_new");
                    CheckBox chk = (CheckBox)Gridview10.Rows[rowIndex].Cells[5].FindControl("Chkcre");
                    DropDownList box3 = (DropDownList)Gridview10.Rows[rowIndex].Cells[6].FindControl("ddkodgst");
                    Label box4 = (Label)Gridview10.Rows[rowIndex].Cells[7].FindControl("Label8");
                    DropDownList box5 = (DropDownList)Gridview10.Rows[rowIndex].Cells[8].FindControl("ddkodgstoth");
                    Label box6 = (Label)Gridview10.Rows[rowIndex].Cells[9].FindControl("Label10");
                    Label box7 = (Label)Gridview10.Rows[rowIndex].Cells[9].FindControl("Label5");

                    box0.SelectedValue = dt.Rows[i]["Column1"].ToString();
                    box1.Text = dt.Rows[i]["Column2"].ToString();
                    box2.Text = dt.Rows[i]["Column3"].ToString();
                    box2_new.Text = dt.Rows[i]["Column4"].ToString();
                    chk.Checked = true;
    ...
    protected void Gridview1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            ddlCountries.DataSource = GetData("select (kod_akaun + ' | '+ nama_akaun) nama_akaun,kod_akaun from KW_ref_carta_akaun where jenis_akaun_type != '1' and ISNULL(kw_acc_header,'0')= '0' and Status='A'");
        ...
        if (ViewState["CurrentTable3"] != null)
        {
            ...
            if (dtCurrentTable3.Rows.Count > 0)
            {
                for (int i = 1; i <= dtCurrentTable3.Rows.Count; i++)
                {
                    ...
                    dtCurrentTable3.Rows[i - 1]["Column4"] = box2_new.Text;
                    dtCurrentTable3.Rows[i - 1]["Column5"] = chk.Checked.ToString();
                    dtCurrentTable3.Rows[i - 1]["Column6"] = box3.SelectedValue;
                    dtCurrentTable3.Rows[i - 1]["Column7"] = box4.Text;
                    dtCurrentTable3.Rows[i - 1]["Column8"] = box5.SelectedValue;
                    dtCurrentTable3.Rows[i - 1]["Column9"] = box6.Text;
                    dtCurrentTable3.Rows[i - 1]["Column10"] = box7.Text;
                    rowIndex++;
                    if (box2.Text != "")
                    {
    ...
    void Pembakal()
    {
        ...
        try
        {
            string com = "select Ref_no_syarikat,ref_nama_syarikat from  kw_ref_pembekal where Status='A' ";
            SqlDataAdapter adpt = new SqlDataAdapter(com, con);
            DataTable dt = new DataTable();
            adpt.Fill(dt);
    ...
    void get_pembekal()
    {
        ...
        {
            string com = "select '01' sno,no_invois from KW_Pembayaran_invois where Bayar_kepada='" + ddpela2.SelectedValue + "' group by no_invois union all"
                + " select '02' sno,no_ruj as no_invois from (select mhn_no_permohonan from KW_Pembayaran_mohon_item where mhn_byr_kepada='" + ddpela2.SelectedValue + "') as a outer apply(select * from KW_Pembayaran_mohon where no_permohonan=mhn_no_permohonan) as b group by no_ruj";
            SqlDataAdapter adpt = new SqlDataAdapter(com, con);
            DataTable dt = new DataTable();
            adpt.Fill(dt);
            ddinv.DataSource = dt;
    ...
    void crt_notelink()
    {
        ...
        DataTable dt = new DataTable();
        dt = Dbcon.Ora_Execute_table("select perkera,Terma,nama_pembekal_code,id_profile_syarikat,no_invois,no_Rujukan,Format(tarkih_mohon,'dd/MM/yyyy') tarikh_invois ,Format(tarikh_kredit,'dd/MM/yyyy') tarikh_credit,project_kod,Overall as jumlah,no_invois,Terma from KW_Pembayaran_Credit where no_Rujukan='" + ssv2 + "'");

        if (dt.Rows.Count > 0)
        {
           
            txttcinvois.Text = dt.Rows[0]["tarikh_credit"].ToString();
            txttcinvois.Attributes.Add("disabled", "disabled");

            TextBox9.Text = dt.Rows[0]["no_Rujukan"].ToString();
            TextBox9.Attributes.Add("disabled", "disabled");

            TextBox4.Text = dt.Rows[0]["perkera"].ToString();
            TextBox4.Attributes.Add("disabled", "disabled");

            ddakaun.SelectedValue = dt.Rows[0]["id_profile_syarikat"].ToString();
            ddakaun.Attributes.Add("disabled", "disabled");

            ddpela2.SelectedValue = dt.Rows[0]["nama_pembekal_code"].ToString();
            ...
            ddinv.SelectedItem.Text = dt.Rows[0]["no_invois"].ToString();
            ...
            query1 = "select a.*,((a.tot - a1.krd) + a2.deb) as baki from (select no_invois,UPPER(keterangan) as Keteragan,s3.Bank_Name as bank,s1.nv_noacc_bank as acc_no,unit as jumlah, discount as Disk,s1.quantiti as qty, "
                         + " UPPER(ISNULL(s4.Ref_nama_cukai,'')) as jen_cukai, s1.gstjumlah as cukai_amt, s1.jumlah as amount,s1.Overall as tot  from KW_Pembayaran_invoisBil_item s1  "
                         + " left join Ref_Nama_Bank s3 on s3.Bank_Code=s1.inv_bank  "
                         + " left join KW_Ref_Tetapan_cukai s4 on s4.Ref_kod_cukai=s1.tax where no_invois='" + dt.Rows[0]["no_invois"].ToString() + "' and s1.Status='A') as a"
                         + " outer apply (select sum(Overall) krd from KW_Pembayaran_Credit where no_invois=a.no_invois and kew_appr_gl_cd='L') as a1 "
                         + " outer apply (select ISNULL(sum(Overall),'0.00') deb from KW_Pembayaran_Dedit where no_invois=a.no_invois and kew_appr_gl_cd='L') as a2"
                         + " union all  select a.*,((a.tot - ISNULL(a1.krd,'0.00')) + ISNULL(a2.deb,'0.00')) as baki from (select m1.no_ruj as no_invois,UPPER(Catatan) as Keteragan,s3.Bank_Name as bank,s1.mhn_noacc_bank as acc_no,mhn_amount as jumlah, mhn_disc as Disk,'1' as qty, "
                    + " UPPER(ISNULL(s4.Ref_nama_cukai, '')) as jen_cukai, s1.mhn_tax as cukai_amt, s1.mhn_amount as amount,s1.mhn_amount_bsr as tot from KW_Pembayaran_mohon_item s1 "
                    + " inner join KW_Pembayaran_mohon m1 on m1.no_permohonan = s1.mhn_no_permohonan left join Ref_Nama_Bank s3 on s3.Bank_Code = s1.mhn_bank "
                    + " left join KW_Ref_Tetapan_cukai s4 on s4.Ref_kod_cukai = s1.mhn_jns_tax where m1.no_ruj = '" + dt.Rows[0]["no_invois"].ToString() + "' and s1.Status = 'A') as a "
                    + " outer apply (select sum(Overall) krd from KW_Pembayaran_Credit where no_invois = a.no_invois and kew_appr_gl_cd = 'L') as a1 "
                    + " outer apply (select ISNULL(sum(Overall), '0.00') deb from KW_Pembayaran_Dedit where no_invois = a.no_invois and kew_appr_gl_cd = 'L') as a2";
            ...
            dt1 = Dbcon.Ora_Execute_table("select kod_akauan,s1.nama_akaun ,s3.Ref_kod_cukai as gsttype,s3.Ref_nama_cukai as gstname,keterangan,CAST(CASE c.tax WHEN 'Y' THEN 1 ELSE 0 END AS BIT) AS tax,othgst,case when c.gstjumlah = '0.00' then '0.00' else Format(c.gstjumlah,'#,##,###.00') end as gstjumlah , '0.00' as othgstjumlah , case when c.jumlah = '0.00' then '0.00' else Format(c.jumlah,'#,##,###.00') end as jumlah , case when c.overall = '0.00' then '0.00' else Format(c.overall,'#,##,###.00') end as overall   from KW_Pembayaran_Credit_item c left join KW_Ref_Carta_Akaun s1 on s1.kod_akaun=c.kod_akauan left join KW_Ref_Tetapan_cukai s3 on s3.Ref_kod_cukai=c.tax where no_Rujukan='" + ssv2 + "'");
            Gridview12.DataSource = dt1;
            Gridview12.DataBind();

            //DataTable get_sum_item = new DataTable();
            //get_sum_item = Dbcon.Ora_Execute_table("select sum(overall) amt from KW_Pembayaran_Credit_item where no_Rujukan='" + ssv2 + "' and Status='A'");
    ...
    void bind_invois()
    {
        string com = "select '01' sno,no_invois from KW_Pembayaran_invois where Bayar_kepada='" + ddpela2.SelectedValue + "' and no_invois='" + ddinv.SelectedValue + "' group by no_invois union all"
              + " select '02' sno,no_ruj as no_invois from (select mhn_no_permohonan from KW_Pembayaran_mohon_item where mhn_byr_kepada='" + ddpela2.SelectedValue + "') as a outer apply(select * from KW_Pembayaran_mohon where no_permohonan=mhn_no_permohonan and no_ruj='" + ddinv.SelectedValue + "') as b group by no_ruj";
        DataTable ddokdicno = new DataTable();
        ddokdicno = DBCon.Ora_Execute_table(com);
        if (ddokdicno.Rows[0]["sno"].ToString() == "01")
        {
            lnk_clk_crt();
        }
        else
    ...
    void lnk_clk_crt()
    {
        ...
        DataTable dt = new DataTable();
        DataTable dt2 = new DataTable();
        dt = Dbcon.Ora_Execute_table("select * from KW_Pembayaran_invois where no_invois='" + ssv1 + "' and Bayar_kepada='" + ddpela2.SelectedValue + "'");
        DataTable dt1 = new DataTable();
        if (dt.Rows.Count > 0)
        {
            ...
            ddakaun.SelectedValue = dt.Rows[0]["id_profile_syarikat"].ToString();
            ddpela2.SelectedValue = dt.Rows[0]["Bayar_kepada"].ToString().Trim();
            
            string query1 = string.Empty, query2 = string.Empty;
            Gridview9.Visible = true;
            Gridview12.Visible = false;
            query1 = "select a.*,((ISNULL(a.tot,'0.00') - ISNULL(a1.krd,'0.00')) + ISNULL(a2.deb,'0.00')) as baki from (select s1.no_invois,UPPER(keterangan) as Keteragan,s3.Bank_Name as bank,s1.nv_noacc_bank as acc_no,unit as jumlah, discount as Disk,s1.quantiti as qty, "
                         + " UPPER(ISNULL(s4.Ref_nama_cukai,'')) as jen_cukai, s1.gstjumlah as cukai_amt, s1.jumlah as amount,s1.Overall as tot,m1.project_kod,m1.perkara,m1.Terma  from KW_Pembayaran_invoisBil_item s1  "
                         + " inner join KW_Pembayaran_invois m1 on m1.no_invois=s1.no_invois "
                         + " left join Ref_Nama_Bank s3 on s3.Bank_Code=s1.inv_bank  "
        ...
        + " from kw_pembayaran_invois m1 left join KW_General_Ledger m2 on m2.GL_invois_no = m1.no_invois and m2.Gl_jenis_no = m1.no_invois and "
        + " m2.kod_akaun = (select top(1)* from(select kod_akaun from KW_General_Ledger where GL_invois_no='"+ ssv1 + "' and ref2='01' group by kod_akaun except select * from (select Ref_kod_akaun from KW_Ref_Pembekal group by Ref_kod_akaun union all select Ref_kod_akaun from kw_ref_tetapan_cukai where tc_jenis='PEM' and Status='A' group by Ref_kod_akaun union all select Ref_kod_akaun from KW_Ref_Diskaun where tc_jenis='PEM' and Status='A' group by Ref_kod_akaun ) as a1) as a order by a.kod_akaun desc ) "
        + " where m1.no_invois = '" + ssv1 + "'  ) as a  outer apply    (select sum(Overall) krd from KW_Pembayaran_Credit "
        + " where no_invois = a.inv and kew_appr_gl_cd = 'L') as a1  outer apply   (select ISNULL(sum(Overall), '0.00') deb from KW_Pembayaran_Dedit where no_invois = a.inv and kew_appr_gl_cd = 'L') as a2 ";
    ...
    void lnk_clk_crt1()
    {
        ...
        DataTable dt = new DataTable();
        DataTable dt2 = new DataTable();
        dt = Dbcon.Ora_Execute_table("select id_profile_syarikat,Bayar_kepada from (select mhn_no_permohonan,mhn_byr_kepada as Bayar_kepada from KW_Pembayaran_mohon_item where mhn_byr_kepada='" + ddpela2.SelectedValue +"') as a outer apply(select * from KW_Pembayaran_mohon where no_permohonan=mhn_no_permohonan) as b where no_ruj='"+ ssv1 + "' ");
        DataTable dt1 = new DataTable();
        if (dt.Rows.Count > 0)
        {
            ...
            Gridview9.Visible = true;
            Gridview12.Visible = false;
            query1 = "  select a.*,((a.tot - ISNULL(a1.krd,'0.00')) + ISNULL(a2.deb,'0.00')) as baki from (select m1.no_ruj as no_invois,UPPER(Catatan) as Keteragan,s3.Bank_Name as bank,s1.mhn_noacc_bank as acc_no,mhn_amount as jumlah, mhn_disc as Disk,'1' as qty, "
                    + " UPPER(ISNULL(s4.Ref_nama_cukai, '')) as jen_cukai, s1.mhn_tax as cukai_amt, s1.mhn_amount as amount,s1.mhn_amount_bsr as tot,s1.mhn_projek_cd project_kod, m1.perkara,m1.Terma from KW_Pembayaran_mohon_item s1 "
                    + " inner join KW_Pembayaran_mohon m1 on m1.no_permohonan = s1.mhn_no_permohonan left join Ref_Nama_Bank s3 on s3.Bank_Code = s1.mhn_bank "
                    + " left join KW_Ref_Tetapan_cukai s4 on s4.Ref_kod_cukai = s1.mhn_jns_tax where m1.no_ruj = '"+ssv1+"' and s1.Status = 'A') as a "
                    + " outer apply (select sum(Overall) krd from KW_Pembayaran_Credit where no_invois = a.no_invois and kew_appr_gl_cd = 'L') as a1 "
                    + " outer apply (select ISNULL(sum(Overall), '0.00') deb from KW_Pembayaran_Dedit where no_invois = a.no_invois and kew_appr_gl_cd = 'L') as a2";
            dt1 = Dbcon.Ora_Execute_table(query1);
            if (dt1.Rows.Count != 0)
            ...
            Gridview9.DataBind();
            Gridview10.Visible = true;
            query2 = " select a.Column1,a.Column2,sum(cast(a.column3 as money)) column3,a.Column4,a.Column5,a.Column6,sum(cast(a.Column7 as money)) Column7,a.Column8,sum(cast(a.Column9 as money)) Column9,sum(a.Column10) Column10 from ( select  a.Column1,a.Column2,((ISNULL(a.Column3,'0.00') - iSNULL(a1.krd,'0.00')) + ISNULL(a2.deb,'0.00')) column3,a.Column4,a.Column5,'' Column6,'0.00' Column7,'' Column8,'0.00'  "
    + "  Column9,((ISNULL(a.Column6,'0.00') - ISNULL(a1.krd,'0.00')) + ISNULL(a2.deb,'0.00')) Column10 from (select GL_invois_no inv, ISNULL(kod_akaun,'') as Column1, '' Column2,  "
     + " case when ISNULL(m1.kew_appr_gl_cd,'') = 'L' then  cast(replace((KW_kredit_amt - KW_Debit_amt), '-', '') as money) else (m1.jumlah) end as Column3, '' Column4, '' Column5,  "
      + " case when ISNULL(m1.kew_appr_gl_cd,'') = 'L' then cast(replace((KW_kredit_amt - KW_Debit_amt), '-', '') as money) else (m1.jumlah) end as Column6  "
     + " from kw_pembayaran_mohon m1 left join KW_General_Ledger m2 on m2.GL_invois_no = m1.no_ruj  and "
     + " m2.kod_akaun = (select top(1)* from(select kod_akaun from KW_General_Ledger where GL_invois_no='"+ ssv1 + "' and ref2='03' group by kod_akaun except select * from (select Ref_kod_akaun from KW_Ref_Pembekal where status='A' group by Ref_kod_akaun union all select Ref_kod_akaun from kw_ref_tetapan_cukai where tc_jenis='PEM' and Status='A' group by Ref_kod_akaun union all select Ref_kod_akaun from KW_Ref_Diskaun where tc_jenis='PEM' and Status='A' group by Ref_kod_akaun ) as a1) as a order by a.kod_akaun desc ) "
      + " where m1.no_ruj='" + ssv1 + "'  ) as a  outer apply	(select sum(Overall) krd from KW_Pembayaran_Credit  "
     + " where no_invois=a.inv and kew_appr_gl_cd='L') as a1  outer apply	(select ISNULL(sum(Overall),'0.00') deb from KW_Pembayaran_Dedit where no_invois=a.inv and kew_appr_gl_cd='L')  "
     + " as a2 )  as a group by a.Column1,a.Column2,a.Column4,a.Column5,a.Column6,a.Column8 ";
        ...
        //{
            if (ddpela2.SelectedValue != "")
            {
                if (TextBox9.Text != "")
                {
                    if (txttcinvois.Text != "")
                    {
                        if (TextBox4.Text != "")
                        {
                            if (txttcre.Text != "")
                            {
                                ...
                                DataTable gen_invkd = new DataTable();
                                gen_invkd = Dbcon.Ora_Execute_table("select no_Rujukan from KW_Pembayaran_Credit where no_Rujukan='" + TextBox9.Text + "'");
                                if (gen_invkd.Rows.Count == 0)
                                {
                                    //float jum_unit = 0;
                                    ...
                                    //    jum_unit += float.Parse(total);

                                    //}
                                    string Inssql_main = "insert into KW_Pembayaran_Credit (id_profile_syarikat,no_Rujukan,project_kod,Overall,tarkih_mohon,nama_pembekal_code,no_invois,Terma,tarikh_Kredit,perkera,crt_id,cr_dt,Status) values('" + ddakaun.SelectedValue + "','" + TextBox9.Text + "','" + ddpro1.SelectedValue + "','" + TextBox18.Text + "','" + tDate1.ToString("yyyy-MM-dd") + "','" + ddpela2.SelectedValue + "','" + ddinv.SelectedValue + "','" + ddinvday.SelectedValue + "','" + tDate.ToString("yyyy-MM-dd") + "','" + TextBox4.Text + "','" + userid + "','" + DateTime.Now.ToString("yyyy/MM/dd hh:mm:ss") + "','A')";
                                    ...
                                    if (Status == "SUCCESS")
                                    {
                                        ...
                                        if(DDN1 != "--- PILIH ---")
                                        {
                                            ...
                                            string Inssql = "insert into KW_Pembayaran_Credit_item (no_Rujukan,kod_akauan,keterangan,tax,gstjumlah,jumlah,overall,discount,crt_id,cr_dt,Status) values('" + TextBox9.Text + "','" + DD + "','" + KEt + "','" + dd1 + "','" + gamt + "','" + total + "','" + ttotal + "','" + disk + "','" + userid + "','" + DateTime.Now.ToString("yyyy/MM/dd hh:mm:ss") + "','A')";
                    ...
                    {
                        ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "$.Zebra_Dialog('Sila Pilih Tarikh Nota Kredit Kreditor .',{'type': 'warning','title': 'warning','auto_close': 2000});", true);
                    }
    ...
    protected void Bindcredit()
    {
        ...
        {
            condtion = "and a.no_Rujukan ='" + txtcno.Text + "' and s1.Ref_nama_syarikat='" + txtnpembe.Text + "' and a.tarkih_mohon='" + fmdate + "'";
        }
        else if (txtcno.Text == "" && txtctarikh.Text != "" && txtnpembe.Text != "")
        {
            condtion = "and s1.Ref_nama_syarikat='" + txtnpembe.Text + "' and a.tarkih_mohon='" + fmdate + "'";
        }
        else if (txtcno.Text != "" && txtctarikh.Text == "" && txtnpembe.Text != "")
        {
            condtion = "and a.no_Rujukan ='" + txtcno.Text + "' and s1.Ref_nama_syarikat='" + txtnpembe.Text + "'";
        }
        else if (txtcno.Text != "" && txtctarikh.Text != "" && txtnpembe.Text == "")
        {
            condtion = "and a.no_Rujukan ='" + txtcno.Text + "' and a.tarkih_mohon='" + fmdate + "'";
        }
        else if (txtcno.Text != "" && txtctarikh.Text == "" && txtnpembe.Text == "")
        {
            condtion = "and a.no_Rujukan ='" + txtcno.Text + "'";
        }
        else if (txtcno.Text == "" && txtctarikh.Text != "" && txtnpembe.Text == "")
        {
            condtion = "and a.tarkih_mohon='" + fmdate + "'";
        }
        else if (txtcno.Text == "" && txtctarikh.Text == "" && txtnpembe.Text != "")
        {
        ...
        qry1 = "select Format(tarkih_mohon,'dd/MM/yyyy')tarkih_permohonan,a.no_Rujukan,s1.Ref_nama_syarikat as  Bayar_kepada,a.Overall as  jumlah,CASE ISNULL(a.semak_sts,'')  WHEN '' THEN 'WAITING'  WHEN 'P' THEN 'PROSES' WHEN 'L' THEN 'LULUS' END  as status,ISNULL(lulus_sts,'') lul_sts_cd,CASE ISNULL(lulus_sts,'')  WHEN '' THEN 'WAITING'  WHEN 'P' THEN 'PROSES' WHEN 'L' THEN 'LULUS' END  as lul_status, a.perkera keterangan,no_invois  from KW_Pembayaran_Credit a left join KW_Ref_Pembekal s1 on s1.Ref_no_syarikat=a.nama_pembekal_code   where a.Status='A' " + condtion + "";
    ...
    {
        Response.Redirect("../kewengan/kw_pembayaran_kredit.aspx");
    }
    ...
    private void GetUniqueId()
    {
        ...
        //else
        //{
        //    DataTable dt = Dbcon.Ora_Execute_table("select ISNULL(max(SUBSTRING(no_Rujukan,13,2000)),'0') from KW_Pembayaran_Credit");
        //    if (dt.Rows.Count > 0)
        //    {
        //        int seqno = Convert.ToInt32(dt.Rows[0][0].ToString());
        //        int newNumber = seqno + 1;
    ...
    protected void Gridview9_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            System.Web.UI.WebControls.TextBox jum_kec = (System.Web.UI.WebControls.TextBox)e.Row.FindControl("Txtdis");
            ...
            if(kod_akaun.Text != "")
            {
                ddlCountries.SelectedValue = kod_akaun.Text;
            }
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            DataTable ddokdicno = new DataTable();
            ddokdicno = DBCon.Ora_Execute_table("select * From KW_Pembayaran_Credit where no_Rujukan='" + lblTitle.Text + "' and Status='A' ");
            if (ddokdicno.Rows.Count != 0)
            {
                string Inssql_main = "Update KW_Pembayaran_Credit set Status='T' where no_Rujukan='" + lblTitle.Text + "' and Status='A'";
                Status = DBCon.Ora_Execute_CommamdText(Inssql_main);
                if (Status == "SUCCESS")
                {
                    string Inssql_item = "Update KW_Pembayaran_Credit_item set Status='T' where no_Rujukan='" + lblTitle.Text + "' and Status='A'";
                    Status = DBCon.Ora_Execute_CommamdText(Inssql_item);
                    BindMohon();
                    ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "$.Zebra_Dialog('Nota Dabit Kreditor Dihapuskan Berjaya',{'type': 'confirmation','title': 'Success','auto_close': 2000});", true);
                }
            }
        }
        catch
