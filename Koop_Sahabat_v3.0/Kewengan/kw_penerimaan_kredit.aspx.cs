...
public partial class kw_penerimaan_kredit : System.Web.UI.Page
{
    ...
    protected void ddkodgst_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (unit.ToString() != "")
        {
            ...
            if (tgst_1 != "--- PILIH ---")
            {
                ...
                {
                    decimal tot1 = ((Convert.ToDecimal(unit.Text) * numqty) - Convert.ToDecimal(dis.Text)) + Convert.ToDecimal(gmt.Text);
                    int tgst1 = Convert.ToInt32(tgst);
    ...
    void crt_notelink()
    {
        ...
        DataTable dt = new DataTable();
        dt = Dbcon.Ora_Execute_table("select perkara,Terma,inv_bank,inv_noacc_bank,nama_pelanggan_code,id_profile_syarikat,no_invois,no_notakredit as no_Rujukan,'' tarikh_invois ,Format(tarikh_kredit,'dd/MM/yyyy') tarikh_credit,project_kod,Overall as jumlah,no_invois,Terma from KW_Penerimaan_Credit where no_notakredit='" + ssv2 + "'");
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            dt1 = Dbcon.Ora_Execute_table("select '' kod_akauan,'' nama_akaun ,unit,discount,quantiti,s3.Ref_kod_cukai as gsttype,s3.Ref_nama_cukai as gstname,keterangan,CAST(CASE c.tax WHEN 'Y' THEN 1 ELSE 0 END AS BIT) AS tax,othgst,case when c.gstjumlah = '0.00' then '0.00' else Format(c.gstjumlah,'#,##,###.00') end as gstjumlah , '0.00' as othgstjumlah , case when c.jumlah = '0.00' then '0.00' else Format(c.jumlah,'#,##,###.00') end as jumlah , case when c.overall = '0.00' then '0.00' else Format(c.overall,'#,##,###.00') end as overall   from KW_Penerimaan_Credit_item c left join KW_Ref_Tetapan_cukai s3 on s3.Ref_kod_cukai=c.tax where no_notakredit='" + ssv2 + "'");
            Gridview12.DataSource = dt1;
            Gridview12.DataBind();

            //DataTable get_sum_item = new DataTable();
            //get_sum_item = Dbcon.Ora_Execute_table("select sum(Overall) amt from KW_Penerimaan_Credit_item where no_notakredit='" + ssv2 + "' and Status='A'");
                ...
                if (txtnoruju.Text != "")
                {
                    if (TextBox4.Text != "")
                    {
                        ...
                        if (txttcre.Text != "")
                        {
                            ...
                            DataTable gen_invkd = new DataTable();
                            gen_invkd = Dbcon.Ora_Execute_table("select no_notakredit from KW_Penerimaan_Credit where no_notakredit='" + txtnoruju.Text + "'");
                            if (gen_invkd.Rows.Count > 0)
                            {
                                DataTable dtkre_1 = Dbcon.Ora_Execute_table("select doc_type_cd,format as fmt,SUBSTRING(cur_format, 1, 12) as lfrmt1,SUBSTRING(cur_format, 13, 4) as lfrmt2,cur_format as cfmt from KW_Format_Nombor_rujukan where doc_type_cd='10' and Status='A'");
                                ...
                                get_doc = Dbcon.Ora_Execute_table("select Ref_doc_descript as s1 from KW_Ref_Doc_kod where Ref_doc_code='10'");
                                DataTable dtkre_2 = Dbcon.Ora_Execute_table("select    ISNULL(max(SUBSTRING(no_notakredit,13,2000)),'0') from KW_Penerimaan_Credit");
                        ...
                        string Inssql_main = "insert into KW_Penerimaan_Credit (id_profile_syarikat,no_notakredit,project_kod,Overall,nama_pelanggan_code,no_invois,Terma,tarikh_kredit,perkara,inv_bank,inv_noacc_bank,crt_id,cr_dt,Status) values('" + ddakaun.SelectedValue + "','" + txtnoruju_1.Text + "','" + ddpro1.SelectedValue + "','" + TextBox18.Text + "','" + ddpela2.SelectedValue + "','" + ddinv.SelectedValue + "','" + ddinvday.SelectedValue + "','" + tDate1.ToString("yyyy-MM-dd") + "','" + TextBox4.Text + "','"+ DropDownList2.SelectedValue + "','" + TextBox15.Text + "','" + userid + "','" + DateTime.Now.ToString("yyyy/MM/dd hh:mm:ss") + "','A')";
                                ...
                                string Inssql = "insert into KW_Penerimaan_Credit_item (no_notakredit,keterangan,tax,gstjumlah,jumlah,overall,discount,unit,quantiti,crt_id,cr_dt,Status) values('" + txtnoruju_1.Text + "','" + KEt + "','" + dd1 + "','" + gamt + "','" + total + "','" + ttotal + "','" + disk + "','"+ unit + "','" + qty + "','" + userid + "','" + DateTime.Now.ToString("yyyy/MM/dd hh:mm:ss") + "','A')";
                                    ...
                                    {
                                        DataTable dt_upd_format = new DataTable();
                                        dt_upd_format = Dbcon.Ora_Execute_table("update KW_Format_Nombor_rujukan set cur_format='" + txtnoruju_1.Text + "',upd_id='" + userid + "',upd_dt='" + DateTime.Now.ToString("yyyy/MM/dd hh:mm:ss") + "' where doc_type_cd='10' and Status = 'A'");
                                        th2.Visible = false;
                                        th1.Visible = true;
                                        reset();
                                        tab3();
                                        ...
                                        Bindcredit();
                                        Binddedit();
                                        ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "$.Zebra_Dialog('Debtor Nota Kredit Dibuat Berjaya.',{'type': 'confirmation','title': 'Success','auto_close': 2000});", true);
                                    }
                                }
    ...
    protected void Bindcredit()
    {
        ...
        {
            condtion = "and a.no_notakredit ='" + txtcno.Text + "' and s1.Ref_nama_syarikat='" + txtnpembe.Text + "' and a.tarikh_kredit='" + fmdate + "'";
        }
        else if (txtcno.Text == "" && txtctarikh.Text != "" && txtnpembe.Text != "")
        {
            condtion = "and s1.Ref_nama_syarikat='" + txtnpembe.Text + "' and a.tarikh_kredit='" + fmdate + "'";
        }
        else if (txtcno.Text != "" && txtctarikh.Text == "" && txtnpembe.Text != "")
        {
            condtion = "and a.no_notakredit ='" + txtcno.Text + "' and s1.Ref_nama_syarikat='" + txtnpembe.Text + "'";
        }
        else if (txtcno.Text != "" && txtctarikh.Text != "" && txtnpembe.Text == "")
        {
            condtion = "and a.no_notakredit ='" + txtcno.Text + "' and a.tarikh_kredit='" + fmdate + "'";
        }
        else if (txtcno.Text != "" && txtctarikh.Text == "" && txtnpembe.Text == "")
        {
            condtion = "and a.no_notakredit ='" + txtcno.Text + "'";
        }
        else if (txtcno.Text == "" && txtctarikh.Text != "" && txtnpembe.Text == "")
        {
            condtion = "and a.tarikh_kredit='" + fmdate + "'";
        }
        else if (txtcno.Text == "" && txtctarikh.Text == "" && txtnpembe.Text != "")
        {
        ...
        qry1 = "select Format(tarikh_kredit,'dd/MM/yyyy') tarkih_permohonan,a.no_notakredit as no_Rujukan,s1.Ref_nama_syarikat as  Bayar_kepada,a.Overall as  jumlah,CASE ISNULL(a.semak_sts,'')  WHEN '' THEN 'WAITING'  WHEN 'P' THEN 'PROSES' WHEN 'L' THEN 'LULUS' END  as status,ISNULL(lulus_sts,'') lul_sts_cd,CASE ISNULL(lulus_sts,'')  WHEN '' THEN 'WAITING'  WHEN 'P' THEN 'PROSES' WHEN 'L' THEN 'LULUS' END  as lul_status, a.perkara keterangan,no_invois  from KW_Penerimaan_Credit a left join KW_Ref_Pelanggan s1 on s1.Ref_no_syarikat=a.nama_pelanggan_code   where a.Status='A' " + condtion + "";
    ...
    {
        Response.Redirect("../kewengan/kw_penerimaan_kredit.aspx");
    }
    ...
    {
        DataTable dt1 = Dbcon.Ora_Execute_table("select doc_type_cd,format as fmt,SUBSTRING(cur_format, 1, 12) as lfrmt1,SUBSTRING(cur_format, 13, 4) as lfrmt2,cur_format as cfmt from KW_Format_Nombor_rujukan where doc_type_cd='10' and Status='A'");
        if (dt1.Rows.Count != 0)
        {
            if (dt1.Rows[0]["cfmt"].ToString() == "")
            {
        ...
        {
            DataTable get_doc = new DataTable();
            get_doc = Dbcon.Ora_Execute_table("select Ref_doc_descript as s1,s1.ws_format as s2 from KW_Ref_Doc_kod left join site_settings s1 on  s1.ID='1' where Ref_doc_code='10'");
            DataTable dt = Dbcon.Ora_Execute_table("select ISNULL(max(SUBSTRING(no_notakredit,13,2000)),'0') from KW_Penerimaan_Credit");
            if (dt.Rows.Count > 0)
            {
                int seqno = Convert.ToInt32(dt.Rows[0][0].ToString());
                int newNumber = seqno + 1;
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            DataTable ddokdicno = new DataTable();
            ddokdicno = DBCon.Ora_Execute_table("select * From KW_Penerimaan_Credit where no_notakredit='" + lblTitle.Text + "' and Status='A' ");
            if (ddokdicno.Rows.Count != 0)
            {
                string Inssql_main = "Update KW_Penerimaan_Credit set Status='T' where no_notakredit='" + lblTitle.Text + "' and Status='A'";
                Status = DBCon.Ora_Execute_CommamdText(Inssql_main);
                if (Status == "SUCCESS")
                {
                    string Inssql_item = "Update KW_Penerimaan_Credit_item set Status='T' where no_notakredit='" + lblTitle.Text + "' and Status='A'";
                    Status = DBCon.Ora_Execute_CommamdText(Inssql_item);
                    BindMohon();
                    ScriptManager.RegisterStartupScript(this, GetType(), "alertMessage", "$.Zebra_Dialog('Nota Kredit Debtor Dihapuskan Berjaya',{'type': 'confirmation','title': 'Success','auto_close': 2000});", true);
                }
            }
        }
        catch
