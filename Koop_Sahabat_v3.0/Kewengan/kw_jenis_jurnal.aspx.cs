...
public partial class kw_jenis_jurnal : System.Web.UI.Page
{
    ...
    void view_details()
    {
        try
        {
            ...
            ddokdicno = Dblog.Ora_Execute_table("select jur_desc,jur_type_cd,jur_item, status From KW_ref_jurnal_type where id = '" + ogid + "'");
            if (ddokdicno.Rows.Count != 0)
            {
                txt_jenis.Attributes.Add("Readonly", "Readonly");
                txt_jenis.Text = ddokdicno.Rows[0]["jur_type_cd"].ToString();
                TextBox1.Text = ddokdicno.Rows[0]["jur_desc"].ToString();
                dd_list_sts.SelectedValue = ddokdicno.Rows[0]["status"].ToString();
                DropDownList1.SelectedValue = ddokdicno.Rows[0]["jur_item"].ToString();
            }
            else
            {
    ...
    {
        DataTable ddok = new DataTable();
        ddok = Dblog.Ora_Execute_table("select jur_type_cd from KW_ref_jurnal_type where jur_type_cd='"+ txt_jenis.Text+ "'");
        if (ddok.Rows.Count == 0)
        {

            if (txt_jenis.Text != "")
            {
                SqlCommand ins_prof = new SqlCommand("insert into KW_ref_jurnal_type (jur_desc,jur_type_cd,jur_item,Status,cr_dt,crt_id) values(@jur_desc,@jur_type_cd,@jur_item,@Status,@cr_dt,@crt_id)", conn);

                //insert into KW_Jenis_Cara_bayaran(Jenis_bayaran_cd, Jenis_bayaran, Status, cr_dt, crt_id) values(@Jenis_bayaran_cd, @Jenis_bayaran, @Status, @cr_dt, @crt_id

                ins_prof.Parameters.AddWithValue("@jur_desc", TextBox1.Text.Replace("'", "''"));
                ins_prof.Parameters.AddWithValue("@jur_type_cd", txt_jenis.Text.Replace("'", "''"));
                ins_prof.Parameters.AddWithValue("@Status", dd_list_sts.SelectedValue);
                ins_prof.Parameters.AddWithValue("@jur_item", DropDownList1.SelectedValue);
                ...
                Response.Redirect("../kewengan/kw_jenis_jurnal_view.aspx");
    ...
    {
        if (TextBox1.Text != "" && txt_jenis.Text != "" && dd_list_sts.SelectedValue != "" && DropDownList1.SelectedValue!="")
        {

            SqlCommand prof_upd = new SqlCommand("update KW_ref_jurnal_type  set jur_desc=@jur_desc,jur_type_cd=@jur_type_cd,jur_item=@jur_item,Status=@Status,upd_dt=@upd_dt,upd_id=@upd_id where Id=@Id", conn);
            prof_upd.Parameters.AddWithValue("id", gen_id.Text);
            prof_upd.Parameters.AddWithValue("jur_desc", TextBox1.Text.Replace("'", "''"));
            prof_upd.Parameters.AddWithValue("jur_type_cd", txt_jenis.Text.Replace("'", "''"));
            prof_upd.Parameters.AddWithValue("Status", dd_list_sts.SelectedValue);
            prof_upd.Parameters.AddWithValue("@jur_item", DropDownList1.SelectedValue);
    ...
    protected void Button5_Click(object sender, EventArgs e)
    {
        ...
        Session["alrt_msg"] = "";
        Response.Redirect("../kewengan/kw_jenis_jurnal.aspx");
    }
