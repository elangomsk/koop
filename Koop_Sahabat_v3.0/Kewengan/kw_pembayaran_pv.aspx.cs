...
public partial class kw_pembayaran_pv : System.Web.UI.Page
{
    ...
    void lnk_clk_Deb()
    {
        ...
        DataTable dt = new DataTable();
        dt = Dbcon.Ora_Execute_table("select untuk_akaun,cre_kod_akaun,Format(tarkih_invois,'dd/MM/yyyy') tarikh_invois,case when project_kod='--- PILIH ---' then '' else project_kod end as project_kod,Terma,deb_kod_akaun,item,keterangan,Format(unit,'#,##,###.00') unit,quantiti,gst,Format(Overall,'#,##,###.00') jumlah,tax from KW_Pembayaran_invoisBil_item where no_invois='" + ssv1 + "' and cre_kod_akaun='" + ddpela3.SelectedItem.Value + "'");
