...
public partial class kw_pembayaran_debit : System.Web.UI.Page
{
    ...
    void crt_notelink()
    {
        ...
        DataTable dt = new DataTable();
        dt = Dbcon.Ora_Execute_table("select perkera,Terma,nama_pembekal_code,id_profile_syarikat,no_invois,no_Rujukan,Format(tarkih_mohon,'dd/MM/yyyy') tarikh_invois ,Format(tarikh_debit,'dd/MM/yyyy') tarikh_credit,project_kod,Overall as jumlah,no_invois,Terma from KW_Pembayaran_Dedit where no_Rujukan='" + ssv2 + "'");
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            dt1 = Dbcon.Ora_Execute_table("select kod_akauan,s1.nama_akaun ,s3.Ref_kod_cukai as gsttype,s3.Ref_nama_cukai as gstname,keterangan,CAST(CASE c.tax WHEN 'Y' THEN 1 ELSE 0 END AS BIT) AS tax,othgst,case when c.gstjumlah = '0.00' then '0.00' else Format(c.gstjumlah,'#,##,###.00') end as gstjumlah , '0.00' as othgstjumlah , case when c.jumlah = '0.00' then '0.00' else Format(c.jumlah,'#,##,###.00') end as jumlah , case when c.overall = '0.00' then '0.00' else Format(c.overall,'#,##,###.00') end as overall   from KW_Pembayaran_Dedit_item c left join KW_Ref_Carta_Akaun s1 on s1.kod_akaun=c.kod_akauan left join KW_Ref_Tetapan_cukai s3 on s3.Ref_kod_cukai=c.tax where no_Rujukan='" + ssv2 + "'");
            Gridview12.DataSource = dt1;
            Gridview12.DataBind();

            //DataTable get_sum_item = new DataTable();
            //get_sum_item = Dbcon.Ora_Execute_table("select sum(overall) amt from KW_Pembayaran_Dedit_item where no_Rujukan='" + ssv2 + "' and Status='A'");
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ddokdicno = DBCon.Ora_Execute_table("select * From KW_Pembayaran_Dedit where no_Rujukan='" + lblTitle.Text + "' and Status='A' ");
            if (ddokdicno.Rows.Count != 0)
            {
                string Inssql_main = "Update KW_Pembayaran_Dedit set Status='T' where no_Rujukan='" + lblTitle.Text + "' and Status='A'";
                Status = DBCon.Ora_Execute_CommamdText(Inssql_main);
                if (Status == "SUCCESS")
                {
                    string Inssql_item = "Update KW_Pembayaran_Dedit_item set Status='T' where no_Rujukan='" + lblTitle.Text + "' and Status='A'";
    ...
    void lnk_clk_crt()
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            query1 = "select a.*,((a.tot - ISNULL(a1.krd,'0.00')) + ISNULL(a2.deb,'0.00')) as baki from (select s1.no_invois,UPPER(keterangan) as Keteragan,s3.Bank_Name as bank,s1.nv_noacc_bank as acc_no,unit as jumlah, discount as Disk,s1.quantiti as qty, "
                    ...
                    if (txttcinvois.Text != "")
                    {
                        if (TextBox4.Text != "")
                        {
                            if (txttcre.Text != "")
                            {
                                ...
                                DataTable gen_invkd = new DataTable();
                                gen_invkd = Dbcon.Ora_Execute_table("select no_Rujukan from KW_Pembayaran_Dedit where no_Rujukan='" + TextBox9.Text + "'");
                                if (gen_invkd.Rows.Count == 0)
                                {
                                    //float jum_unit = 0;
                                    ...
                                    //}
                                    string Inssql_main = "insert into KW_Pembayaran_Dedit (id_profile_syarikat,no_Rujukan,project_kod,Overall,tarkih_mohon,nama_pembekal_code,no_invois,Terma,tarikh_debit,perkera,crt_id,cr_dt,Status) values('" + ddakaun.SelectedValue + "','" + TextBox9.Text + "','" + ddpro1.SelectedValue + "','" + TextBox18.Text + "','" + tDate1.ToString("yyyy-MM-dd") + "','" + ddpela2.SelectedValue + "','" + ddinv.SelectedValue + "','" + ddinvday.SelectedValue + "','" + tDate.ToString("yyyy-MM-dd") + "','" + TextBox4.Text + "','" + userid + "','" + DateTime.Now.ToString("yyyy/MM/dd hh:mm:ss") + "','A')";
                                    Status = DBCon.Ora_Execute_CommamdText(Inssql_main);
                                    if (Status == "SUCCESS")
                                    {
                                        int count = 0;
                                        ...
                                        string Inssql = "insert into KW_Pembayaran_Dedit_item (no_Rujukan,kod_akauan,keterangan,tax,gstjumlah,jumlah,overall,discount,crt_id,cr_dt,Status) values('" + TextBox9.Text + "','" + DD + "','" + KEt + "','" + dd1 + "','" + gamt + "','" + total + "','" + ttotal + "','" + disk + "','" + userid + "','" + DateTime.Now.ToString("yyyy/MM/dd hh:mm:ss") + "','A')";
    ...
    protected void Bindcredit()
    {
        ...
        qry1 = "select Format(tarkih_mohon,'dd/MM/yyyy')tarkih_permohonan,a.no_Rujukan,s1.Ref_nama_syarikat as  Bayar_kepada,a.Overall as  jumlah,CASE ISNULL(a.semak_sts,'')  WHEN '' THEN 'WAITING'  WHEN 'P' THEN 'PROSES' WHEN 'L' THEN 'LULUS' END  as status,ISNULL(lulus_sts,'') lul_sts_cd,CASE ISNULL(lulus_sts,'')  WHEN '' THEN 'WAITING'  WHEN 'P' THEN 'PROSES' WHEN 'L' THEN 'LULUS' END  as lul_status, a.perkera keterangan,no_invois  from KW_Pembayaran_Dedit a left join KW_Ref_Pembekal s1 on s1.Ref_no_syarikat=a.nama_pembekal_code   where a.Status='A' " + condtion + "";
