...
public partial class kw_inv_kod_industry : System.Web.UI.Page
{
    ...
    void view_details()
    {
        ...
        try
        {
            ...
            ddokdicno = DBCon.Ora_Execute_table("select * From KW_Ref_Kod_Industri where Id='" + lblid1 + "' ");
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                ver_id.Text = "1";
                get_id.Text = lblid1;
                TextBox8.Attributes.Add("Readonly","true");
                TextBox8.Text = ddokdicno.Rows[0]["msic_kod"].ToString();
                TextBox1.Text = ddokdicno.Rows[0]["msic_desc"].ToString();
                DropDownList2.SelectedValue = ddokdicno.Rows[0]["c_sts"].ToString();
            }
            else
            {
    ...
    protected void clk_submit(object sender, EventArgs e)
    {
            if (TextBox8.Text != "")
            {
                if (ver_id.Text == "0")
                {
                    ...
                    ddokdicno1 = DBCon.Ora_Execute_table("select * From KW_Ref_Kod_Industri where msic_kod='" + TextBox8.Text + "'");
                    if (ddokdicno1.Rows.Count == 0)
                    {
                        string Inssql1 = "insert into KW_Ref_Kod_Industri(msic_kod,msic_desc,c_sts,crt_id,crt_dt) values ('" + TextBox8.Text.Replace("'", "''") + "','" + TextBox1.Text.Replace("'", "''") + "','" + DropDownList2.SelectedValue + "','" + Session["New"].ToString() + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "')";
                        Status1 = DBCon.Ora_Execute_CommamdText(Inssql1);
                        if (Status1 == "SUCCESS")
                        {
                        Session["validate_success"] = "SUCCESS";
                        Session["alrt_msg"] = "Rekod Berjaya Disimpan.";
                        Response.Redirect("../kewengan/kw_inv_kod_industry_view.aspx");
                ...
                {
                    string Inssql1 = "UPDATE KW_Ref_Kod_Industri set msic_desc='" + TextBox1.Text.Replace("'", "''") + "',c_sts='" + DropDownList2.SelectedValue + "',upd_id='" + Session["New"].ToString() + "',upd_dt='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' where Id = '" + get_id.Text + "'";
                    Status1 = DBCon.Ora_Execute_CommamdText(Inssql1);
                    if (Status1 == "SUCCESS")
                    {
                    Session["validate_success"] = "SUCCESS";
