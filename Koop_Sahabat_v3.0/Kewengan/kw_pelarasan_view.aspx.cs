...
public partial class kw_pelarasan_view : System.Web.UI.Page
{
    ...
    protected void BindData()
    {
        ...
        SqlCommand cmd = new SqlCommand("select s1.no_rujukan,s1.keterangan,FORMAT(s1.tarikh,'dd/MM/yyyy', 'en-us') tarikh,sum(s1.debit_amt) damt,sum(s1.kredit_amt) kamt from KW_Pelarasan s1 " + sqry + " group by s1.no_rujukan,s1.keterangan,s1.tarikh", con);
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../kewengan/kw_pelarasan.aspx?edit={0}", name));
            }
            else
            {
                Session["validate_success"] = "";
