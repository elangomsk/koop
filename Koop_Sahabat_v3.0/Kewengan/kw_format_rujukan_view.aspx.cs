...
public partial class kw_format_rujukan_view : System.Web.UI.Page
{
...

    {
        if (Session["New"] != null)
        {
            ...
            gt_lng = DBCon.Ora_Execute_table("select " + Session["site_languaage"].ToString() + " from Ref_language where ID IN ('718','705','133','39')");
    ...
    protected void BindData()
    {
        ...
        //else
        //{
        //    sqry = "where Jenis_rujukan like '%" + srch_id.Text + "%'";
        //}

        qry1 = "select s2.Id,Jenis_rujukan,format,s2.status,s1.Ref_doc_name as frm_type,s2.cur_format from KW_Format_Nombor_rujukan s2 left join KW_Ref_Doc_kod s1 on s1.Ref_doc_code=doc_type_cd " + sqry + " order by Jenis_rujukan";
        ...
        if (rcount != "0")
        {
            foreach (GridViewRow row in gv_refdata.Rows)
            {
                ...
                if (rbn.Checked)
                {
                    ...
                    SqlCommand ins_peng = new SqlCommand("Delete from KW_Format_Nombor_rujukan where id='" + varName1 + "'", conn);
                    conn.Open();
    ...
    protected void lnkView_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                Response.Redirect(string.Format("../kewengan/kw_format_rujukan.aspx?edit={0}", name));
