...
public partial class kw_pembekal : System.Web.UI.Page
{
...

    {
        if (Session["New"] != null)
        {
            ...
            gt_lng = DBCon.Ora_Execute_table("select " + Session["site_languaage"].ToString() + " from Ref_language where ID IN ('740','705','1543','709','496','1704','739','1705','1526','1420','1018','29','347','28','1650','1706','824','738','61','15','77')");
            ...
            ps_lbl4.Text = txtinfo.ToTitleCase(gt_lng.Rows[19][0].ToString().ToLower());
            ...
            ps_lbl7.Text = txtinfo.ToTitleCase(gt_lng.Rows[16][0].ToString().ToLower());
            ps_lbl8.Text = txtinfo.ToTitleCase(gt_lng.Rows[10][0].ToString().ToLower());
            ps_lbl9.Text = txtinfo.ToTitleCase(gt_lng.Rows[17][0].ToString().ToLower());    
            ...
            ps_lbl16.Text = txtinfo.ToTitleCase(gt_lng.Rows[20][0].ToString().ToLower());
            ps_lbl17.Text = txtinfo.ToTitleCase(gt_lng.Rows[18][0].ToString().ToLower());
            ps_lbl18.Text = txtinfo.ToTitleCase(gt_lng.Rows[13][0].ToString().ToLower());
            ps_lbl19.Text = txtinfo.ToTitleCase(gt_lng.Rows[8][0].ToString().ToLower());
    ...
    void view_details()
    {
        ...
        try
        {
            ...
            ddokdicno = DBCon.Ora_Execute_table("select * From KW_Ref_Pembekal where Id='" + lblid + "' ");
            if (ddokdicno.Rows.Count != 0)
            {
                ...
                DropDownList1.SelectedValue = ddokdicno.Rows[0]["pem_bank_cd"].ToString();
                TextBox12.Text = ddokdicno.Rows[0]["pem_bank_accno"].ToString();
                TextBox11.Text = ddokdicno.Rows[0]["ref_alamat_ked"].ToString();
                TextBox9.Text = ddokdicno.Rows[0]["pem_bandar"].ToString();
                ddnegeri.SelectedValue = ddokdicno.Rows[0]["pem_negeri"].ToString();
                TextBox14.Text = ddokdicno.Rows[0]["pem_poskod"].ToString();
                if (ddokdicno.Rows[0]["pem_negera"].ToString() != "")
                {
                    dd_negera.SelectedValue = ddokdicno.Rows[0]["pem_negera"].ToString();
                }
                else
                {
                    dd_negera.SelectedValue = "129";
    ...
    protected void clk_submit(object sender, EventArgs e)
    {
        if (TextBox1.Text != "" && TextBox5.Text != "")
        {
            ...
            {
                DataTable ddokdicno = new DataTable();
                ddokdicno = DBCon.Ora_Execute_table("select * From KW_Ref_Pembekal where Ref_nama_syarikat='" + TextBox1.Text + "' and Ref_no_syarikat='" + TextBox5.Text + "'");
                if (ddokdicno.Rows.Count == 0)
                {
                    ...
                    //if (Status == "SUCCESS")
                    //{
                    string Inssql = "insert into KW_Ref_Pembekal(Ref_nama_syarikat,Ref_no_syarikat,Ref_no_telefone,Ref_no_fax,Ref_email,Ref_gst_id,Ref_kod_akaun,Ref_jenis_akaun,Ref_alamat,Status,crt_id,cr_dt,Ref_kawalan,ref_alamat_ked,pem_bandar,pem_negeri,pem_poskod,pem_negera,pem_bank_cd,pem_bank_accno) values ('" + TextBox1.Text.Replace("'", "''") + "','" + TextBox5.Text.Replace("'", "''") + "','" + TextBox4.Text.Replace("'", "''") + "','" + TextBox2.Text.Replace("'", "''") + "','" + TextBox3.Text.Replace("'", "''") + "','" + TextBox6.Text.Replace("'", "''") + "','"+ TextBox10.Text +"','" + ss_kod + "','" + TextBox7.Text.Replace("'", "''") + "','A','" + Session["New"].ToString() + "','" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "','" + TextBox8.Text.Replace("'", "''") + "','" + TextBox11.Text.Replace("'", "''") + "','" + TextBox9.Text.Replace("'", "''") + "','" + ddnegeri.SelectedValue + "','" + TextBox14.Text.Replace("'", "''") + "','" + dd_negera.SelectedValue + "','" + DropDownList1.SelectedValue + "','" + TextBox12.Text + "')";
                        Status = DBCon.Ora_Execute_CommamdText(Inssql);
                        Session["alrt_msg"] = "Rekod Berjaya Disimpan.";
                        Session["validate_success"] = "SUCCESS";
                        Response.Redirect("../kewengan/kw_pembekal_view.aspx");
                    //}
                    //else
                    //{
            ...
            {
                //DataTable ddokdicno = new DataTable();
                //ddokdicno = DBCon.Ora_Execute_table("select * From KW_Ref_Pembekal where Id = '" + get_id.Text + "'");
                ...
                string Inssql = "UPDATE KW_Ref_Pembekal set " + ckod + " pem_bank_cd='" + DropDownList1.SelectedValue + "',pem_bank_accno='" + TextBox12.Text + "',Ref_nama_syarikat='" + TextBox1.Text.Replace("'", "''") + "',Ref_no_syarikat='" + TextBox5.Text.Replace("'", "''") + "',Ref_no_telefone='" + TextBox4.Text.Replace("'", "''") + "',Ref_no_fax='" + TextBox2.Text.Replace("'", "''") + "',Ref_email='" + TextBox3.Text.Replace("'", "''") + "',Ref_gst_id='" + TextBox6.Text.Replace("'", "''") + "',Ref_alamat='" + TextBox7.Text.Replace("'", "''") + "',Ref_kawalan='" + TextBox8.Text.Replace("'", "''") + "', ref_alamat_ked='" + TextBox11.Text.Replace("'", "''") + "',pem_bandar='" + TextBox9.Text.Replace("'", "''") + "',pem_negeri='" + ddnegeri.SelectedValue + "',pem_poskod='" + TextBox14.Text.Replace("'", "''") + "',pem_negera='" + dd_negera.SelectedValue + "',Ref_jenis_akaun='"+ ss_kod + "',Ref_kod_akaun='"+ TextBox10.Text + "',upd_id='" + Session["New"].ToString() + "',upd_dt='" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' where Id = '" + get_id.Text + "'";
                    Status = DBCon.Ora_Execute_CommamdText(Inssql);
    ...
    protected void Button5_Click(object sender, EventArgs e)
    {
        ...
        Session["alrt_msg"] = "";
        Response.Redirect("../kewengan/kw_pembekal.aspx");
    }
