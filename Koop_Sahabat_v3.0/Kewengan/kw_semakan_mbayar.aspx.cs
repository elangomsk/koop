...
public partial class kw_semakan_mbayar : System.Web.UI.Page
{
        ...
        if (txtnoper.Text != "")
        {
            if (txttarkihmo.Text != "" && ddbkepada.SelectedValue != "" && DropDownList2.SelectedValue != "" && DropDownList1.SelectedValue != "")
            {
                ...
                if (ddsts.SelectedValue != "")
                {
                    ...
                    userid = Session["New"].ToString();
                    DataTable dt = new DataTable();
                    string Inssql_main = "update KW_Pembayaran_mohon  set id_profile_syarikat='" + DropDownList3.SelectedValue + "',Catatan='" + TextBox29.Text + "',chk_sts='" + semsts + "',chk_sts_id='" + userid + "',chk_sts_dt='" + DateTime.Now.ToString("yyyy-MM-dd hh: mm:ss") + "',upd_id='" + userid + "',upd_dt='" + DateTime.Now.ToString("yyyy-MM-dd hh: mm:ss") + "' where no_permohonan='" + txtnoper.Text + "' and Status='A'";
                    Status = DBCon.Ora_Execute_CommamdText(Inssql_main);
                    if (Status == "SUCCESS")
                    {
                        BindMohon();
    ...
    protected void change_sts(object sender, EventArgs e)
    {
        if(ddsts.SelectedValue != "")
        {
            TextBox29.Text = DateTime.Now.ToString("dd/MM/yyyy") + "-" + ddsts.SelectedItem.Text + " : ";
        }
    }
    protected void lblSubbind_Click(object sender, EventArgs e)
    {
        ...
        DataTable dt = new DataTable();
        dt = Dbcon.Ora_Execute_table("select no_permohonan,Catatan,id_profile_syarikat,chk_sts,semak_id,Format(semak_dt,'dd/MM/yyyy') semakdt,ID_pemohon,PO_no,DO_no,jumlah,Format(tarkih_invois,'dd/MM/yyyy') as tk_invois,Format(tarkih_permohonan,'dd/MM/yyyy') as tk_mohon,Jenis_permohonan,no_ruj,cara_bayaran,kat_penerima,Terma from KW_Pembayaran_mohon where no_permohonan='" + CommandArgument1 + "' and Status='A'");
        //tab1();
        if (dt.Rows.Count != 0)
        {
            if (dt.Rows[0]["chk_sts"].ToString() == "L")
            {
                kem_Button2.Visible = false;
                ddsts.SelectedValue = dt.Rows[0]["chk_sts"].ToString();
            }
            else
            {
                kem_Button2.Visible = true;
    ...
    {
        Response.Redirect("../kewengan/kw_semakan_mbayar.aspx");
    }
    protected void BindMohon()
    {
        ...
        qry1 = "select kat_penerima,Format(tarkih_permohonan,'dd/MM/yyyy')tarkih_permohonan,no_permohonan,b.mhn_byr_kepada as  Bayar_kepada,a.jumlah as  jumlah,CASE ISNULL(a.chk_sts,'')  WHEN '' THEN 'WAITING'  WHEN 'P' THEN 'PROSES' WHEN 'L' THEN 'LULUS' END  as status,ISNULL(lulus_sts,'') lul_sts_cd,CASE ISNULL(lulus_sts,'')  WHEN '' THEN 'WAITING'  WHEN 'P' THEN 'PROSES' WHEN 'L' THEN 'LULUS' END  as lul_status,a.no_ruj,b.mhn_keterangan  from KW_Pembayaran_mohon a OUTER APPLY (select top(1)* from KW_Pembayaran_mohon_item s2 where s2.Status='A' and s2.mhn_no_permohonan=a.no_permohonan order by cr_dt asc) as b  where a.Status='A' " + condtion + "";
